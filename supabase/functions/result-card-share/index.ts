import {
  ...
  serviceClient,
  sha256,
} from "../_shared/resultCardShare.ts";
...
async function createShare(
  ...
  const requestId = request.headers.get("x-result-card-request-id") ?? "",
    token = request.headers.get("x-result-card-token") ?? "";
  if (
  ...
  const { data: reservation, error } = await service.rpc(
    "reserve_result_card_share",
    {
      ...
      p_content_sha256: await sha256(png),
      p_byte_size: png.byteLength,
    },
  );
  if (error || !reservation) return json({ code: "unavailable" }, 503, headers);
  if (reservation.status === "ready") {
    return json({ token, expiresAt: reservation.expiresAt }, 201, headers);
  }
  if (reservation.status !== "reserved") {
    const outcomes: Record<string, [number, string]> = {
      pending: [202, "share_pending"],
      expired: [410, "share_expired"],
      conflict: [409, "request_conflict"],
      daily_limit: [429, "daily_limit"],
      capacity_reached: [503, "capacity_reached"],
      cleanup_unhealthy: [503, "cleanup_unhealthy"],
    };
  ...
  const upload = await service.storage.from(BUCKET).upload(
  ...
  const published = await service.rpc("publish_result_card_share", {
    p_id: reservation.id,
  });
  if (published.error) return json({ code: "unavailable" }, 503, headers);
  if (published.data?.status !== "ready") {
    return json({ code: "share_expired" }, 410, headers);
  }
...
async function readShare(
  ...
  const service = serviceClient();
  const { data, error } = await service.from("result_card_shares").select(
    "object_path,expires_at",
  ).eq("token_hash", await sha256(token)).eq("state", "ready").not(
    "owner_id",
    "is",
    null,
  ...
  headers.set("x-result-card-expires-at", data.expires_at);
...
function responseHeaders(request: Request): Headers {
  ...
  const origins = (Deno.env.get("RESULT_CARD_ALLOWED_ORIGINS") ??
    "https://jinhoops.github.io,http://localhost:5173").split(",").map((v) =>
      v.trim()
    );
  ...
  headers.set(
    ...
    "authorization,content-type,x-result-card-request-id,x-result-card-token,x-result-card-share",
  );
  headers.set(
    "access-control-expose-headers",
    "x-result-card-expires-at,retry-after",
  );
