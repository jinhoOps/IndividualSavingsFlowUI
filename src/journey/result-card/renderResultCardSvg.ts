...
export function renderResultCardSvg(model: ResultCardModel): string {
  ...
  const allocationRows = model.allocation.rows.map((row, index) => {
    ...
    const y = 1138 + rowIndex * 37;
    return `<circle cx="${x + 8}" cy="${y - 5}" r="7" fill="${row.color}"/><text x="${x + 26}" y="${y}" textLength="360" lengthAdjust="spacingAndGlyphs" class="allocation-name">${escapeXml(row.name)}</text><text x="${x + 430}" y="${y}" text-anchor="end" class="allocation-value">${escapeXml(row.percentage)}</text>${row.amount ? `<text x="${x + 430}" y="${y + 21}" text-anchor="end" class="allocation-detail">${escapeXml(row.amount)}</text>` : ''}`;
  }).join('');
  ...
  const chart = path(model.growth.points, 'plan', 570, 790, 440, 170, false);
  const baseline = path(model.growth.points, 'baseline', 570, 790, 440, 170, true);
  ...
  ${model.growth.rows.map((row, index) => `<text x="50" y="680" class="body">${escapeXml(row.label)}</text><text x="470" y="680" text-anchor="end" class="value">${escapeXml(row.value)}</text>`).join('')}
  <line x1="570" x2="1010" y1="960" y2="960" class="line"/><path d="${baseline}" fill="none" stroke="#a4b0b6" stroke-width="6" stroke-dasharray="12 12"/><path d="${chart}" fill="none" stroke="#247f79" stroke-width="8"/>
  <line x1="50" x2="1030" y1="1018" y2="1018" class="line"/>
  <text x="50" y="1068" class="section">03 투자 배분</text><text x="50" y="1110" class="headline">${escapeXml(model.allocation.headline)}</text>${allocationRows}
...
function rowText(row: {label: string; value: string}, y: number): string {
  return `<text x="50" y="${y + 40}" class="body">${escapeXml(row.label)}</text><text x="500" y="${y + 40}" text-anchor="end" class="value">${escapeXml(row.value)}</text>`;
}
