...
export function AccountMapModal({ node, related, sourceElement, fallbackElement, reducedMotion, recovery, recoveryPending, saveFailed, onClose, onReapply, onKeepLatest, onSaveEdit, locations = [], onConnectLocation, onCreateAndConnectLocation, initialMode = 'read', purposeParentLabel, purposeTargetCapacityWon, onArchivePurpose, onRestorePurpose, onArchiveLocation, onRestoreLocation }: AccountMapModalProps): JSX.Element {
  ...
  const [restorePurposeTarget, setRestorePurposeTarget] = useState(String(node.amountWon ?? 0));
    ...
    <div className="account-map-modal-backdrop" onPointerDown={(event) => { if (event.target === event.currentTarget) requestClose(); }}>
      <div ref={modalRef} className="account-map-modal" role="dialog" aria-modal="true" aria-labelledby={titleId} aria-busy={animating || undefined}>
        ...
        <div className="account-map-modal__body">
          ...
          {recovery.status === 'none' ? null : <div className="account-map-modal__error" role={recovery.status === 'collision' || recovery.status === 'manual' ? 'alert' : 'status'}><p id={recoveryDescriptionId}>{modalRecoveryMessage(recovery)}</p><div className="account-map-actions"><button ref={replayRef} type="button" className="ui-button ui-button--primary" aria-describedby={recoveryDescriptionId} disabled={recoveryPending} onClick={() => { void onReapply().then((saved) => { if (saved) { if (mode === 'archive-purpose') returnToFallbackRef.current = true; requestClose(true); } }); }}>{recovery.status === 'manual' ? '최신 상태에서 다시 검토' : '최신 상태에서 다시 적용'}</button><button type="button" className="ui-button ui-button--secondary" disabled={recoveryPending} onClick={() => requestClose()}>최신 값 유지</button></div></div>}
        </div>
        <footer>
          {mode === 'read' ? <>{node.kind === 'location' && node.status === 'suspended' && onRestoreLocation !== undefined ? <button type="button" className="ui-button ui-button--secondary" disabled={recovery.status !== 'none'} onClick={() => setMode('restore-location')}>복원</button> : null}{node.kind === 'location' && node.status !== 'suspended' && onArchiveLocation !== undefined ? <button type="button" className="ui-button ui-button--secondary account-map-modal__archive" disabled={recovery.status !== 'none'} onClick={() => setMode('archive-location')}><TrashIcon />보관</button> : null}<button type="button" className="ui-button ui-button--primary" disabled={animating || recovery.status !== 'none'} onClick={() => setMode('edit')}>편집</button></> : null}
          ...
          {mode === 'archive-location' ? <><button type="button" className="ui-button ui-button--secondary" disabled={actionPending || recoveryPending} onClick={() => { if (recovery.status === 'none') setMode('read'); else requestClose(); }}>취소</button><button type="button" className="ui-button ui-button--primary" disabled={recovery.status !== 'none' || replacementMissing || actionPending} onClick={() => {
          ...
          {mode === 'archive-purpose' ? <><button type="button" className="ui-button ui-button--secondary" disabled={actionPending || recoveryPending} onClick={() => { if (recovery.status === 'none') setMode('read'); else requestClose(); }}>취소</button><button type="button" className="ui-button ui-button--primary" disabled={recovery.status !== 'none' || actionPending || !node.id.startsWith('custom:')} onClick={() => commitPurposeLifecycle(node.id.startsWith('custom:') && onArchivePurpose !== undefined ? () => onArchivePurpose(node.id as `custom:${string}`) : undefined, true)}>{actionError ? '다시 시도' : '보관하기'}</button></> : null}
          {mode === 'restore-purpose' ? <><button type="button" className="ui-button ui-button--secondary" disabled={actionPending || recoveryPending} onClick={() => { if (recovery.status === 'none' && initialMode === 'read') setMode('read'); else requestClose(); }}>취소</button><button type="button" className="ui-button ui-button--primary" disabled={recovery.status !== 'none' || actionPending || !node.id.startsWith('custom:') || !Number.isSafeInteger(Number(restorePurposeTarget)) || Number(restorePurposeTarget) < 0 || (purposeTargetCapacityWon !== undefined && Number(restorePurposeTarget) > purposeTargetCapacityWon)} onClick={() => commitPurposeLifecycle(node.id.startsWith('custom:') && onRestorePurpose !== undefined ? () => onRestorePurpose(node.id as `custom:${string}`, Number(restorePurposeTarget)) : undefined, true)}>{actionError ? '다시 시도' : '목적 복원'}</button></> : null}
        </footer>
      </div>
    </div>
  );
