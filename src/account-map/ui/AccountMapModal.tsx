...
export interface AccountMapNodeEditInput {
  label?: string;
  locationDetails?: FinancialLocationFieldsValue;
  targetMonthlyWon?: number;
  links: Array<{
    id: string;
...
export function AccountMapModal({
  node,
  related,
  sourceElement,
  fallbackElement,
  ...
  const directRelated = related.filter(
  ...
  const locationRecord =
    node.kind !== "location"
      ? undefined
      : locations.find(({ id }) => id === node.id.replace(/^location:/u, ""));
  const defaultLocationFields: FinancialLocationFieldsValue = {
    kind: locationRecord?.kind ?? "bank",
    ...(locationRecord?.institution === undefined
      ? {}
      : { institution: locationRecord.institution }),
    shortName: locationRecord?.shortName ?? node.label,
  };
  const defaultEditTarget = node.kind === "purpose" && node.id.startsWith("custom:")
    ? formatWonInput(node.amountWon ?? 0, { zeroDisplay: "zero" })
    : "";
  const defaultEditLinks = (locationOnly ? [] : directRelated)
  ...
  const recoveredLinks = recovered === null
    ? defaultEditLinks
    : defaultEditLinks.map((item) => {
      const saved = recovered.editLinks.find(({ id }) => id === item.id);
      return saved === undefined ? item : {
        ...item,
        monthlyAmountWon: formatWonInput(parseWonInput(saved.monthlyAmountWon), { zeroDisplay: "zero" }),
        status: saved.status,
        remainder: saved.remainder,
      };
    });
  const [locationFields, setLocationFields] =
    useState<FinancialLocationFieldsValue>(recovered?.locationFields ?? defaultLocationFields);
  const [editTarget, setEditTarget] = useState(recovered?.editTarget ?? defaultEditTarget);
  const [editLinks, setEditLinks] = useState(() => recoveredLinks);
  const [editReplacementByPurpose, setEditReplacementByPurpose] = useState<
    Record<string, string | null>
  >(recovered?.editReplacementByPurpose ?? {});
  const [replacementByPurpose, setReplacementByPurpose] = useState<
    Record<string, string | null>
  >({});
  ...
  const [restoreRemainderByPurpose, setRestoreRemainderByPurpose] = useState<
    Record<string, string | null>
  >({});
  const [restorePurposeTarget, setRestorePurposeTarget] = useState(
    formatWonInput(node.amountWon ?? 0, { zeroDisplay: "zero" }),
  );
  ...
  const editDirty = editLabel !== node.label
    || JSON.stringify(locationFields) !== JSON.stringify(defaultLocationFields)
    || editTarget !== defaultEditTarget
    || JSON.stringify(editLinks) !== JSON.stringify(defaultEditLinks)
    || Object.keys(editReplacementByPurpose).length > 0;
  ...
  useEffect(() => {
    ...
    const sourceRect =
      sourceElement?.isConnected === true
  ...
  useEffect(() => {
    ...
    mode,
    recovery.status,
    recovery.status === "collision" ? collisionIdentity(recovery) : "",
  ]);
  ...
  useEffect(() => {
    ...
    modalRef.current
      ?.querySelector<HTMLElement>(
        ".account-map-modal__body button:not(:disabled), .account-map-modal__body input:not(:disabled), .account-map-modal__body select:not(:disabled), footer button:not(:disabled)",
      )
      ?.focus();
  }, [mode]);
  ...
  function restoreFocus() {
    const focusTarget =
      !returnToFallbackRef.current && sourceElement?.isConnected === true
        ? sourceElement
        : fallbackElement;
    if (focusTarget !== null) {
    ...
    const targetRect =
      sourceElement?.isConnected === true
        ...
        : (fallbackElement?.getBoundingClientRect() ??
          modal.getBoundingClientRect());
    animationRef.current = animateModalToNode(modal, targetRect, {
      reducedMotion,
  ...
  async function commitConnection(
    action: (() => Promise<boolean> | boolean) | undefined,
  ): Promise<boolean> {
    if (action === undefined || actionPending || recovery.status !== "none")
      return false;
  ...
  function commitPurposeLifecycle(
    action: (() => Promise<boolean> | boolean) | undefined,
  ...
  const remainderPurposes = [
    ...new Set(
      archiveImpacts
        .filter(({ remainder }) => remainder === true)
  ...
  const replacementMissing = remainderPurposes.some((purposeId) => {
    const candidates = related.filter(
      (item) =>
        item.replacementCandidate === true && item.purposeId === purposeId,
    );
    return candidates.length > 0 && !replacementByPurpose[purposeId];
  });
  const restoreExcessPurposes = [
    ...new Set(
      restorableRelated
        .filter(
          (item) =>
            item.linkId !== undefined &&
            restoreLinkIds.includes(item.linkId) &&
            item.purposeId !== undefined,
        )
        .map((item) => item.purposeId!),
    ),
  ].filter((purposeId) => {
    const target = restorableRelated.find(
      (item) => item.purposeId === purposeId,
    )?.purposeTargetWon;
    if (target === undefined) return false;
    const restoring = restorableRelated
      .filter(
        (item) =>
  ...
  const editRemainderPurposes = [
    ...
    (purposeId) =>
      editLinks.some(
        (item) => item.purposeId === purposeId && item.status === "active",
      ) ||
      related.some(
        (item) =>
          item.replacementCandidate === true &&
          item.purposeId === purposeId &&
          item.status === "active",
      ),
  );
  const editReplacementMissing = editRemainderPurposes.some(
    (purposeId) =>
      !editLinks.some(
        (item) =>
          item.purposeId === purposeId &&
  ...
  const originalLocationNeedsRepair = locationRecord !== undefined
    && locationRecord.kind !== "cash"
    && locationRecord.institution === undefined;
  const changedLocationInstitution = locationRecord !== undefined
    && (locationRecord.kind !== locationFields.kind
      || !sameInstitution(locationRecord.institution, locationFields.institution));
  const locationEditComplete = locationRecord === undefined
    || isFinancialLocationFieldsComplete(locationFields)
    || (originalLocationNeedsRepair && !changedLocationInstitution);
  const editLinksChanged = editLinks.some((item) => {
    const original = directRelated.find(({ linkId }) => linkId === item.id);
    return original === undefined
      || original.amountWon !== parseWonInput(item.monthlyAmountWon)
      || original.status !== item.status
      || (original.remainder ?? false) !== item.remainder;
      ...
      >
        <header>
          <div>
            ...
            <h2 id={titleId}>{modalTitle(node.label, mode)}</h2>
          </div>
          <div className="account-map-modal__title-actions">
            {node.kind === "purpose" &&
            node.id.startsWith("custom:") &&
            mode === "read" ? (
              <div className="account-map-modal__title-menu">
                <button
                  type="button"
                  className="account-map-modal__more"
                  aria-label={`${node.label} 더보기`}
            ...
            <button
              type="button"
              className="account-map-modal__close"
              aria-label="닫기"
              disabled={animating || actionPending || recoveryPending}
        ...
        <div className="account-map-modal__body">
          ...
          {mode === "read" ? (
            <div className="account-map-modal__connections">
              <h3>연결</h3>
              {directRelated.length === 0 ? (
                <p>연결된 항목이 없습니다.</p>
              ) : (
                directRelated.map((item, index) => (
                  <div key={`${item.label}:${index}`}>
                    <span>{item.label}</span>
                    <strong>{formatWon(item.amountWon)}</strong>
                    {item.status === "suspended" ? <small>중지됨</small> : null}
                  </div>
                ))
              )}
            </div>
              ...
              {node.kind === "location" && locationRecord !== undefined ? (
                <FinancialLocationFields
                  ...
                  onChange={(next) => {
                    ...
                    setEditLabel(next.shortName);
                  }}
                  mode="edit"
                  showValidation
                  shortNameInputProps={{
                    "data-recovery-field": "shortName",
                    "aria-describedby":
                      recovery.status === "collision" && recovery.field === "shortName"
                        ? recoveryDescriptionId
                        : undefined,
                  }}
                />
              ) : node.kind === "location" || node.id.startsWith("custom:") ? (
                <label>
                  표시 이름
                  <input
                    data-recovery-field={
                      node.kind === "location" ? "shortName" : "name"
                    }
                    aria-describedby={
                    ...
                    value={editLabel}
                    maxLength={node.kind === "location" ? 8 : 24}
                    onChange={(event) => setEditLabel(event.target.value)}
                  />
                </label>
              ) : null}
              {node.kind === "purpose" && node.id.startsWith("custom:") ? (
                <label>
                  월 목표 금액
                  <FormattedMoneyInput
                    data-recovery-field="targetMonthlyWon"
                    aria-describedby={
                      recovery.status === "collision" &&
                      recovery.field === "targetMonthlyWon"
                        ? recoveryDescriptionId
                        : undefined
                    }
                    valueWon={parseWonInput(editTarget)}
                    onValueWonChange={(valueWon) => setEditTarget(formatWonInput(valueWon, { zeroDisplay: "zero" }))}
                    zeroDisplay="zero"
                  />
                </label>
              ) : null}
                  ...
                  >
                    ...
                    <label>
                      ...
                      <FormattedMoneyInput
                        ...
                        aria-describedby={
                          collides && recovery.field === "monthlyAmountWon"
                            ? recoveryDescriptionId
                            : undefined
                        }
                          ...
                          setEditLinks((current) =>
                            current.map((row, rowIndex) =>
                              rowIndex === index
                                ? {
                                    ...row,
                                    monthlyAmountWon: formatWonInput(valueWon, { zeroDisplay: "zero" }),
                                  }
                                : row,
                            ),
                          )
                    ...
                    <label>
                      연결 상태
                      <select
                        data-recovery-field="status"
                        data-recovery-link-id={item.id}
                        aria-describedby={
                          collides && recovery.field === "status"
                            ? recoveryDescriptionId
                            : undefined
                        }
                        aria-label={`${item.label} 연결 상태`}
                        value={item.status}
                        onChange={(event) =>
                          setEditLinks((current) =>
                            current.map((row, rowIndex) =>
                      ...
                      >
                        ...
                        <option value="removed">연결 제거</option>
                      </select>
                    </label>
                    {item.status === "active" ? (
                      <label className="account-map-modal__remainder">
                        <input
                          data-recovery-field="remainder"
                          data-recovery-link-id={item.id}
                          aria-describedby={
                            collides && recovery.field === "remainder"
                              ? recoveryDescriptionId
                              : undefined
                          }
                          type="checkbox"
              ...
              {editRemainderPurposes
                ...
                .map((purposeId) => {
                  ...
                  return candidates.length === 0 ? (
                    <p
                      key={`edit-remainder:${purposeId}`}
                      className="account-map-modal__error"
                    >
                      다른 활성 연결을 나머지로 선택해 주세요.
                    </p>
                  ) : (
                    <label key={`edit-remainder:${purposeId}`}>
                      새 나머지 연결
                      <select
                        required
                        aria-label="편집 나머지 연결"
                        value={editReplacementByPurpose[purposeId] ?? ""}
                        onChange={(event) =>
                          setEditReplacementByPurpose((current) => ({
                            ...current,
          ...
          {mode === "connect" ? (
            <div className="account-map-modal__connect">
              <AccountMapLocationPicker
                ...
                amountRequired={directRelated.some(
                  ({ status }) => status === "active",
                )}
                disabled={
                  recovery.status !== "none" || actionPending || recoveryPending
                }
                cancelDisabled={actionPending || recoveryPending}
                onCancel={() => {
                  if (recovery.status === "none") setMode("edit");
                  else requestClose();
                }}
                onSelect={(id, amount) =>
                  commitConnection(
                    onConnectLocation === undefined
                      ? undefined
                      : () => onConnectLocation(id, amount),
                  )
                }
                onCreate={(location, amount) =>
                  commitConnection(
                    onCreateAndConnectLocation === undefined
                      ? undefined
                      : () => onCreateAndConnectLocation(location, amount),
                  )
                }
              />
            </div>
          ) : null}
          {mode === "archive-location" || mode === "archive-purpose" ? (
            <div className="account-map-modal__impact">
              ...
              {mode === "archive-location"
                ? remainderPurposes.map((purposeId) => {
                    const candidates = related.filter(
                      (item) =>
                        item.replacementCandidate === true &&
                        item.purposeId === purposeId,
                    ...
                    return candidates.length === 0 ? null : (
                      <label key={purposeId}>
                        ...
                        <select
                          required
                          aria-label="새 나머지 계좌"
                          value={replacementByPurpose[purposeId] ?? ""}
                          onChange={(event) =>
                            setReplacementByPurpose((current) => ({
                              ...current,
                              [purposeId]: event.target.value || null,
                            }))
                          }
          ...
          {mode === "restore-location" ? (
            <div className="account-map-modal__restore">
              <p>
                보관할 때 중지된 연결만 선택해 다시 연결할 수 있습니다. 선택하지
                않으면 계좌·보관처만 복원합니다.
              </p>
              {restorableRelated.map((item) => (
                <label key={item.linkId}>
                  <input
                    type="checkbox"
                    checked={
                      item.linkId !== undefined &&
                    ...
                    onChange={(event) => {
                      if (item.linkId === undefined) return;
                      setRestoreLinkIds((current) =>
                        event.target.checked
                          ? [...current, item.linkId!]
                          : current.filter((id) => id !== item.linkId),
                      );
                    }}
                  />
              ...
              {restoreExcessPurposes.map((purposeId) => {
                ...
                return (
                  <label
                    key={`restore:${purposeId}`}
                    className="account-map-modal__restore-correction"
                  >
                    초과 금액을 맞출 나머지 연결
                    <select
                      required
                      aria-label="복원 나머지 연결"
                      value={restoreRemainderByPurpose[purposeId] ?? ""}
                      onChange={(event) =>
                        setRestoreRemainderByPurpose((current) => ({
                          ...current,
                          [purposeId]: event.target.value || null,
                        }))
                      }
          ...
          {mode === "restore-purpose" ? (
            <div className="account-map-modal__restore-purpose">
              {purposeParentLabel === undefined ? null : (
                <p>큰 목적 · {purposeParentLabel}</p>
              )}
              <label>
                월 목표 금액
                <FormattedMoneyInput
                  ...
                  valueWon={parseWonInput(restorePurposeTarget)}
                  onValueWonChange={(valueWon) => setRestorePurposeTarget(formatWonInput(valueWon, { zeroDisplay: "zero" }))}
                  zeroDisplay="zero"
                />
              </label>
              {purposeTargetCapacityWon === undefined ? null : (
                <p className="account-map-hint">
                  복원 가능 {formatWon(purposeTargetCapacityWon)}
                </p>
              )}
              {purposeTargetCapacityWon !== undefined &&
              parseWonInput(restorePurposeTarget) > purposeTargetCapacityWon ? (
                <p className="account-map-modal__error">
                  최신 큰 목적의 월 금액을 넘을 수 없습니다.
                </p>
              ) : null}
              {directRelated.some(({ status }) => status === "suspended") ? (
                <p>목적만 복원하며 중지된 연결은 자동으로 재개하지 않습니다.</p>
              ) : null}
            </div>
          ) : null}
          ...
          {saveFailed && recovery.status !== "none" ? (
            <p className="account-map-modal__error" role="alert">
              저장하지 못했습니다. 편집 중인 입력은 그대로 두었습니다.
            </p>
          ) : null}
          {recovery.status === "none" ? null : (
            <div
              className="account-map-modal__error"
              role={
                recovery.status === "collision" || recovery.status === "manual"
                  ? "alert"
                  : "status"
              }
            >
              <p id={recoveryDescriptionId}>{modalRecoveryMessage(recovery)}</p>
              <div className="account-map-actions">
                <Button
                  ...
                  onClick={() => {
                    void onReapply().then((saved) => {
                      if (saved) {
                        if (mode === "archive-purpose")
                          returnToFallbackRef.current = true;
                        requestClose(true);
                      }
                    });
                  }}
          ...
          {mode === "read" ? (
            <>
              ...
              <Button
                variant="primary"
                type="button"
                disabled={animating || recovery.status !== "none"}
                  ...
                  editLinks.some(
                    (item) =>
                      ...
                      !Number.isSafeInteger(
                        parseWonInput(item.monthlyAmountWon),
                      ),
                  )
                }
                onClick={() => {
                  ...
                  void Promise.resolve(
                    onSaveEdit({
                      ...(node.kind === "location" && locationRecord !== undefined
                        ? {
                            label: locationFields.shortName.trim(),
                            locationDetails: {
                              kind: locationFields.kind,
                              shortName: locationFields.shortName.trim(),
                              ...(locationFields.institution === undefined ? {} : { institution: locationFields.institution }),
                            },
                          }
                        : node.kind === "location" || node.id.startsWith("custom:")
                      ...
                      ...(node.kind === "purpose" &&
                      node.id.startsWith("custom:")
                        ? { targetMonthlyWon: parseWonInput(editTarget) }
                        : {}),
                      links: [
                        ...editLinks.map((item) => ({
                          id: item.id,
                          monthlyAmountWon: parseWonInput(
                            item.monthlyAmountWon,
                          ),
                          status: item.status,
                          remainder: item.remainder,
                        })),
                        ...Object.values(editReplacementByPurpose).flatMap(
                          (linkId) => {
                            const item = related.find(
                            ...
                            return item?.linkId === undefined
                              ...
                              : [
                                  {
                                    id: item.linkId,
                                    monthlyAmountWon: item.amountWon,
          ...
          {mode === "archive-location" ? (
            <>
              ...
              <Button
                ...
                onClick={() => {
                  if (locationId === null || onArchiveLocation === undefined)
                    return;
                  setActionError(false);
                  setActionPending(true);
                  void Promise.resolve(
                    onArchiveLocation(locationId, replacementByPurpose),
                  )
                    .then(
                      (saved) => {
                        if (saved) {
          ...
          {mode === "restore-location" ? (
            <>
              <Button
                ...
                disabled={actionPending || recoveryPending}
                onClick={() => {
                  if (recovery.status === "none" && initialMode === "read")
                    setMode("read");
                  else requestClose();
                }}
              >
              ...
              <Button
                ...
                disabled={
                  recovery.status !== "none" ||
                  restoreExcessPurposes.some(
                    (purposeId) => !restoreRemainderByPurpose[purposeId],
                  ) ||
                  actionPending
                }
                onClick={() => {
                  if (locationId === null || onRestoreLocation === undefined)
                    return;
                  setActionError(false);
                  setActionPending(true);
              ...
              >
                {actionError ? "다시 시도" : "선택 복원"}
              </Button>
            </>
          ) : null}
          {mode === "archive-purpose" ? (
            <>
              <Button
                variant="secondary"
                type="button"
              ...
              <Button
                ...
                onClick={() =>
                  commitPurposeLifecycle(
                    node.id.startsWith("custom:") &&
                      onArchivePurpose !== undefined
                      ? () => onArchivePurpose(node.id as `custom:${string}`)
                      : undefined,
                    true,
                  )
                }
            ...
            <>
              ...
              <Button
                ...
                disabled={
                  ...
                  actionPending ||
                  !node.id.startsWith("custom:") ||
                  !Number.isSafeInteger(parseWonInput(restorePurposeTarget)) ||
                  parseWonInput(restorePurposeTarget) < 0 ||
                  (purposeTargetCapacityWon !== undefined &&
                    parseWonInput(restorePurposeTarget) >
                      purposeTargetCapacityWon)
                }
                onClick={() =>
                  commitPurposeLifecycle(
                    node.id.startsWith("custom:") &&
                      onRestorePurpose !== undefined
                      ? () =>
                          onRestorePurpose(
                            node.id as `custom:${string}`,
                            parseWonInput(restorePurposeTarget),
                          )
                      : undefined,
                    true,
...
interface NodeRecovery {
  ...
  editReplacementByPurpose: Record<string, string | null>;
}

function parseNodeRecovery(value: unknown): NodeRecovery | null {
  if (!isRecord(value)
    || typeof value.editLabel !== "string"
    || typeof value.editTarget !== "string"
    || !isLocationFields(value.locationFields)
    || !Array.isArray(value.editLinks)
    || !isNullableStringRecord(value.editReplacementByPurpose)) return null;
  const editLinks = value.editLinks.flatMap((item) => {
    if (!isRecord(item)
      || typeof item.id !== "string"
      || typeof item.monthlyAmountWon !== "string"
      || (item.status !== "active" && item.status !== "suspended" && item.status !== "removed")
      || typeof item.remainder !== "boolean") return [];
    return [{
      id: item.id,
      monthlyAmountWon: item.monthlyAmountWon,
      status: item.status as "active" | "suspended" | "removed",
      remainder: item.remainder,
    }];
  });
  if (editLinks.length !== value.editLinks.length) return null;
  return {
    editLabel: value.editLabel,
    locationFields: value.locationFields,
    editTarget: value.editTarget,
    editLinks,
    editReplacementByPurpose: value.editReplacementByPurpose,
  };
}
...
function sameInstitution(
  left: FinancialLocation["institution"],
  right: FinancialLocation["institution"],
): boolean {
...
function modalTitle(label: string, mode: AccountMapModalMode): string {
  ...
  if (mode === "archive-location" || mode === "archive-purpose")
    return `${label} 보관`;
  return `${label} 복원`;
}

function modalRecoveryMessage(
  recovery: Exclude<RecoveryState, { status: "none" }>,
): string {
  if (recovery.status === "stale")
    return "다른 곳에서 변경된 최신 상태를 불러왔습니다. 편집 중인 입력은 그대로 두었습니다.";
  if (recovery.status === "manual")
    return recovery.reason === "removal"
  ...
  if (recovery.reason === "duplicate-link")
    return "최신 상태에 같은 연결이 이미 있습니다. 최신 값을 유지하거나 입력을 다시 확인해 주세요.";
  if (recovery.reason === "target-missing")
    return "편집 대상이 최신 상태에 없습니다. 최신 값을 유지하거나 입력을 다시 확인해 주세요.";
...
function collisionIdentity(
  recovery: Extract<RecoveryState, { status: "collision" }>,
): string {
  const target =
    recovery.intent.kind === "add-link"
