...
- 금액 행은 `월 지출`, `남는 돈`, `월 저축`, `월 투자` 순서로 색상·명칭·금액·비율을 함께 보여줍니다. 주거·생활비 상세와 음수·초과 문구도 해당 행에서 읽습니다. 명칭·비율은 도넛을 탐색합니다. 저축·투자 금액은 같은 월 금액 편집기를 열어 해당 입력란과 빠른 금액 조정에 초점을 연결하고, 닫으면 진입 금액으로 초점을 돌려줍니다. 지출 금액은 지팡이 아이콘과 `항목별로 계산` 안내로 도우미를 열고, 남는 돈 금액은 `저축·투자에 나누기` 안내로 분배 도우미를 엽니다. 지출 행의 접근성 설명에 주거·생활비 상세를 연결합니다. 다섯 금액 편집은 공통 `월 금액 편집`으로도 열고 행마다 수정 아이콘을 반복하지 않습니다. 767px 이하는 화면 아래 고정된 접힌 바, 768px 이상은 요약 카드 하단 버튼으로 배치합니다. 바는 탭·키보드 진입점이며 드래그 동작은 제공하지 않습니다. 모든 조작 영역은 44px 이상을 유지합니다.
