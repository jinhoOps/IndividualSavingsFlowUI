...
- 페이지 이탈 확인은 실제 미저장 초안·미완료 입력·서버 응답 대기 또는 실패 상태에만 표시합니다. 조회, 이미지 금액 표시 옵션, 샘플 탐색과 원래 값으로 되돌린 입력은 변경으로 취급하지 않습니다. Supabase 저장이 확정된 초안은 페이지 이탈을 막지 않으며, 편집기에서 초안을 명시적으로 버리는 확인은 별도로 유지합니다.
...
- Main·Portfolio 설정 진행 막대와 금액·비율·차트 값은 기존 product spring 모션을 유지합니다. 공통 dialog surface는 Anime.js spring의 잔여 움직임까지 450ms로 재생하며 모바일 sheet는 아래에서 이동하고 desktop modal은 중앙 scale만 사용합니다. 직접 드래그 중에는 손가락 위치를 보간하지 않으며, 금융 값은 목표를 넘지 않습니다. 연속 변경은 현재 표시값에서 이어지며 접근성용 값은 즉시 갱신하고 reduced-motion·초기화 실패에서는 최종 상태를 표시합니다. 하단 `미래 성장 보기`의 기존 CSS 등장 효과와 Simulation·브랜드 시작 장면은 기존 계약을 유지합니다.
...
- Main 소유 편집기는 제목·닫기·설명 뒤에 다섯 금액을 label–input 행으로 표시합니다. 단위는 입력 오른쪽에 두고, 빠른 금액 조정은 focus된 행에서만 펼칩니다. 오류는 해당 행 아래에 연결합니다. 공통 반응형 표면은 모바일에서 bottom sheet, 768px 이상에서 중앙 modal로 표시합니다.
- Main의 모바일 월 금액 편집·지출 계산·남는 돈 분배 sheet는 손잡이와 비상호작용 여백에서 아래로 끌어 닫을 수 있습니다. 드래그는 기존 폐기 확인·저장 guard를 거치며 본문 스크롤과 입력은 계속 동작합니다. 승인된 닫기는 공통 450ms 표면 모션 후 원래 trigger로 focus를 돌려주며, reduced-motion에서는 지연 없이 닫습니다.
...
- 모바일 bottom sheet와 768px 이상 중앙 modal은 배경 비활성화·focus trap·Escape·진입 금액으로 focus 복원을 제공합니다. footer 합계와 행동은 고정하고 질문/13개 내역만 내부 스크롤합니다. 저장 결과 불명과 충돌은 표면 안에서 재시도할 수 있습니다.
...
- 0원·적자는 현재 배분 상태만 설명하며 추가 금액을 받지 않습니다. 모바일 sheet와 desktop modal은 지출 도우미의 overlay·키보드·focus 계약을 따릅니다.
...
- 하단 Portfolio 샘플 링크는 처음에는 문구·버튼·공간을 숨깁니다. Main과 같은 탐색 감지로 페이지 끝에서 새로 시작한 추가 아래 스크롤·위쪽 스와이프 또는 키보드 focus 후에만 드러나며 활성화 전에는 이동·저장하지 않습니다. 조건 편집·관리 메뉴·내부 스크롤에서는 탐색을 무시합니다. 5%·9%·13% 샘플 연결은 유지합니다.
...
- 결과는 목표 도달 핵심 문장과 조건 한 줄 뒤에 하나의 자산 변화 패널을 제공합니다. 패널은 `N년 동안의 자산 변화` 제목(0년은 `현재 자산`)과 현재 명목·실질 기준, 그래프, 낮은 강조의 비교값, 하단 중앙의 투명한 `조건 편집` 버튼 순서로 읽힙니다. 버튼은 44px 이상이며 모바일에서는 Main 편집기와 같은 Anime.js surface spring으로 아래에서 올라오는 하단 시트, 768px 이상에서는 정지 상태의 중앙 모달을 열고 닫으면 버튼으로 focus를 돌려줍니다. 그래프·조작·비교에 중첩 카드 외곽선을 만들지 않습니다. 비교 배율은 `넣은 돈 대비 N배`로 표시합니다.
...
- 조건 편집은 제목·닫기 다음에 명목·실질 `SegmentedControl`을 첫 제어로 보여주고, 기간·기대수익률과 `목표와 가정`을 이어서 제공합니다. `목표와 가정`은 기본으로 접고 현재 목표·시작 자산을 요약합니다. 펼치면 목표·시작 자산·금리·물가를 한 번에 조정하며 중첩된 펼침 UI는 만들지 않습니다. 목표는 1천만 원 단위 직접 입력과 `−5천만 / −1천만 / +1천만 / +5천만`으로 조정하고 기본 목표로 복원합니다. 결과에는 그래프 제목 옆의 현재 기준을 표시합니다. 금액 입력은 모바일 한 열·768px 이상 두 열이며 조작 영역은 최소 44px입니다.
...
- 결과 읽기 폭은 최대 40rem이며 배분 막대와 목록 사이를 8px로 묶습니다. 목록은 큰 카드 외곽선 없이 옅은 행 구분선을 사용하고, 한 줄 기준 금액 숨김은 최소 56px·표시는 최소 68px로 표시합니다. 긴 대상명은 행 높이를 늘려 읽을 수 있게 하며 44px 이상 조작 영역을 유지합니다. 이름은 왼쪽, 비율·보조 금액은 오른쪽에 정렬하고 비율은 20px로 표시합니다.
- 배분 막대는 숫자와 같은 Anime.js value spring으로 변경되며 연속 변경 시 표시 중인 비율에서 이어집니다. 하나의 진행률로 구간을 함께 보간하고 overshoot를 막습니다. 종목 추가·삭제로 구간 구성이 바뀌면 막대·숫자는 최종 비율을 함께 표시하고 행 등장·이동만 전환합니다. 모션 감소·초기화 실패에서는 최종 비율을 즉시 표시합니다.
- 결과 화면은 금액보다 배분 비율을 먼저 보여줍니다. 상단 핵심 요약은 `안정 N%`이며 100% 배분 막대와 이름·비율 목록이 대상별 비중을 전달합니다.
- 기본 상태에서는 총액과 항목별 원화 금액을 숨깁니다. 사용자가 관리 메뉴에서 `금액 보기`를 켜도 비율을 주 정보로 유지하며 총액과 항목별 금액을 보조 정보로 표시합니다.
- 배분 항목은 도넛과 표를 중복하지 않고, 모든 항목을 잇는 32px 100% 가로 막대와 그 아래 이름·비율 목록 하나로 제공합니다. 막대는 장식으로 숨기고 실제 이름·비율·선택 상태는 목록에서 제공하며, 각 행은 44px 이상 pointer·touch·keyboard로 동등하게 확인할 수 있어야 합니다. 같은 대상은 정렬을 바꿔도 같은 색을 유지하고 현금은 고정 중립색을 사용합니다.
- 결과 화면은 별도 연필·수정 버튼 없이 배분 목록의 각 행 전체를 눌러 배분 편집을 엽니다. 행의 hover·focus는 대응하는 막대 구간을 강조하고 44px 이상 조작 영역, 대상 이름과 현재 비율·선택적 금액의 접근성 설명을 유지합니다. 편집을 닫으면 진입 행으로, 그 행이 교체되었으면 첫 결과 행으로 초점을 돌립니다. 지속적인 `저장됨` 문구는 표시하지 않고 저장 오류는 편집 맥락에서 분명히 알립니다.
- 적용된 결과 목록 끝에는 배경·윤곽 없는 44px 이상의 `저장하기`와 `공유하기`를 둡니다. 두 action은 같은 3:4 PNG 미리보기를 열며, 저장은 로컬 다운로드만 하고 공유는 사용자가 `공유 링크 만들기`를 명시적으로 누른 뒤에만 확인한 PNG를 올립니다. 공유 미리보기에는 링크 보유자 공개 범위와 `공유 링크는 최대 2일 동안 열 수 있어요. 정확한 만료 시각은 생성 후 표시돼요.`를 표시합니다. 금액 제외 모드는 원화 수치를 출력 모델에서 제거하며, 링크 수신 화면은 로그인·편집·workspace 조회 없이 이미지와 만료 안내만 표시합니다. 완료한 공유는 이후 계획 수정으로 바뀌거나 만료가 연장되지 않습니다.
- 정렬은 관리 메뉴에서 `비율순`과 `입력순`을 전환하며 기본값은 `비율순`입니다. 같은 비율이면 현재 표시 순서를 유지합니다.
- 저장·공유 이미지의 글자는 가로로 늘이거나 압축하지 않습니다. 미래 자산 항목을 각각 독립된 행에 표시하고, 긴 투자 대상명은 최대 두 줄·말줄임으로 제한하며 비율·금액과 겹치지 않게 배치합니다. 금액 포함 옵션을 바꾸는 동안 기존 미리보기 공간을 유지하고 새 PNG가 완성되기 전에는 저장·공유를 비활성화합니다.
- 편집 화면에서 각 항목을 `성장` 또는 `안정`으로 분류합니다. 현금은 항상 `안정`이며 금현물·채권·금 또는 금·채권 관련 ETF 이름은 `안정`을 자동 추천하되 사용자가 바꿀 수 있고 사용자 지정이 자동 추천보다 우선합니다.
- 배분 단계와 `배분 수정`은 기준 투자금, 성장·안정·현금 요약, 대상 요약 목록, 현금, `+ 투자 대상 추가`, 적용 footer 순서로 전체 배분을 먼저 읽게 합니다. 자유 편집에서는 사용자가 원화 금액을 입력하고 비율은 전체 투자금 기준으로 자동 계산합니다. 대상 행 또는 추가 action은 부모 배분 편집 영역 안에서 한 대상 입력 폼으로 전환하며, 새 오른쪽 panel이나 중첩 dialog를 만들지 않습니다. 분류는 이름 기반 자동 추천을 기본으로 하되 두 선택지가 동시에 보이는 성장·안정 `SegmentedControl`로 바꾸면 사용자 지정이 자동 추천보다 우선합니다. 신규 추가에만 `S&P 500`, `나스닥`, `코스피`, `미국 국채`, `금 현물` 빠른 이름 선택을 보이고, 선택은 이름을 채운 뒤 금액 field로 focus를 옮깁니다. 자동 분류 상태에서는 선택한 이름으로 추천을 갱신하고 사용자 지정 분류는 보존합니다. 금액 빠른 조정은 추가·수정에 모두 제공합니다. 대상 폼에는 이름·금액 `원`·계산 비율·분류 출처와 `취소 / 완료`를 두며, 변경 후 Escape·취소에는 폐기 확인을 요구합니다. 최초 설정의 검토와 재편집 적용 확인은 성장·안정 합계와 각 대상·현금의 금액 및 계산된 비율을 함께 보여줍니다.
- 배분 단계와 `배분 수정`의 보조 경로는 정적 샘플 7개와 직접 조합을 제공합니다. 샘플은 방어 지향·성장 지향·공격 지향의 상대적 탐색 구간과 배당·인컴/인덱스/인덱스 레버리지/금/BTC 속성으로 읽으며, 이 구분은 손실 위험 점수나 투자 권유가 아닙니다. 금(GOLD)은 특정 상품 티커가 아닌 금 자산을 뜻합니다. 선택과 비율 조정은 임시 상태에만 두고, `이 구성으로 채우기`의 명시적 확인 뒤에만 기존 aggregate draft를 한 번 교체합니다. 질문형 투자 성향 탐색은 이 흐름의 선행 조건이 아닌 후속 선택 기능입니다.
- 공통 모바일 sheet는 Anime.js `opacity + translateY`로 아래에서 올라오고, header·본문·footer의 비상호작용 공간에서도 아래로 끌어 닫을 수 있습니다. 본문은 먼저 스크롤되며, 처음부터 맨 위에서 시작한 아래 드래그만 닫기로 처리합니다. 짧은 드래그는 손가락 위치에서 복귀하고 기준을 넘긴 드래그는 기존 닫기·폐기 확인 경로를 실행합니다. 입력·버튼·링크·토글·슬라이더는 드래그 시작점에서 제외합니다.
- `배분 수정`은 결과와 시각적으로 분리된 집중 화면을 엽니다. 767px 이하는 최대 88dvh 하단 sheet, 768px 이상은 손잡이 없는 중앙 modal이며 body만 스크롤하고 결과 영역은 편집 중 비활성화합니다. 대상 편집도 같은 최상위 dialog의 단계로 전환하며, 목록 행에서 대상 편집을 완료·취소하면 그 행으로, 신규 추가 완료 후에는 새 행으로, 신규 추가 취소·삭제 후에는 `+ 투자 대상 추가`로 focus를 돌려보냅니다.
- 변경 전에는 적용 action을 숨기고 첫 변경 뒤에만 취소·적용을 제공합니다.
- 배분 편집의 닫기·취소·Escape·바깥 클릭과 모바일 sheet 상단 드래그는 같은 종료 경로를 사용합니다. 항목 인라인 입력 중 Escape는 부모 편집기를 닫지 않고 항목 입력만 종료합니다. 변경이 없으면 닫고, 초안·미완료 현금 입력·선택한 샘플 변경이 있으면 `계속 수정 / 변경 버리기` 확인을 제공합니다. 패널 안 여백 클릭은 닫기가 아닙니다. 저장 중에는 종료를 막습니다.
- 샘플 탐색은 제목·뒤로가기를 한 곳에 두고 1100px 이상에서 목록·상세를 나란히, 그보다 좁으면 목록 → 상세로 보여줍니다. 768px 이하 샘플 탐색은 전체 높이 화면이며 일반 배분 편집 sheet와 구분합니다. 목록·상세 본문을 스크롤하고 하단 `이 구성으로 초안 채우기`는 항상 접근 가능하게 유지합니다. Simulation의 5%·9%·13% CTA는 각각 SCHD 50·금 50, QQQM 70·SCHD 30, QLD 50·SCHD 30·금 20 상세를 자동으로 먼저 선택하며, 자동 선택만으로 dirty가 되지 않습니다. 선택한 비율 그대로 초안을 만들고 기존 초안 교체는 같은 화면에서 확인합니다. 상세 Escape는 목록으로, 목록 Escape는 배분 편집으로 돌아갑니다. [결과·샘플 탐색 설계](docs/superpowers/specs/2026-09-16-portfolio-overview-and-sample-navigation-design.md)를 따릅니다.
...
- 편집, 설정, 결과 이미지 미리보기는 native `dialog`를 사용합니다. 767px 이하는 화면 아래에 붙는 최대 88dvh sheet, 768px 이상은 손잡이 없는 중앙 modal입니다.
- 표면은 손잡이·제목·뒤로/닫기 header, 선택 context, 유일한 scroll body, 상태, footer를 같은 순서로 배치합니다. footer는 body를 가리지 않으며 일반 조작 영역은 44px 이상입니다.
- sheet는 opacity와 아래에서 올라오는 이동을, desktop modal은 opacity와 중앙 scale을 사용합니다. modal은 수직 이동하지 않습니다. Anime.js spring 잔여 움직임까지 포함한 목표는 450ms이며 브라우저 측정은 400–500ms 범위입니다. 550ms 타이머는 애니메이션 복구용 상한이며 정상 완료 시간으로 사용하지 않습니다. `prefers-reduced-motion`과 모션 초기화 실패에서는 즉시 최종 상태를 표시합니다.
- body는 유일한 스크롤 영역입니다. 스크롤을 우선하고 맨 위에서 새로 시작한 아래 드래그만 sheet를 이동합니다. handle뿐 아니라 비상호작용 header·body·footer 공간도 시작점으로 허용하고 입력·버튼·링크·토글·slider는 제외합니다.
- 데스크톱 form·wide modal은 각각 `min(46rem, calc(100dvh - 48px))`·`min(54rem, calc(100dvh - 48px))` 캔버스를 유지합니다. 내부 form helper가 focus로 펼쳐지거나 접혀도 footer와 action target 위치를 고정하기 위한 규격입니다. Compact dialog는 내용 높이를 따릅니다.
- Portfolio 대상과 샘플은 같은 dialog의 단계로 바꾸며, 폐기·적용처럼 확인이 필요한 경우에만 하나의 최상위 confirmation dialog를 엽니다. 열린 부모와 자식에 두 focus trap을 만들지 않습니다.
...
- 톱니 설정 surface는 앱별 보기 설정·다시 설정 뒤에 공통 계정 정보·로그아웃을 표시합니다. 일반 백업 내보내기·가져오기와 중복된 `앱 아이콘 안내`는 표시하지 않습니다. 초기 브라우저 이전과 오류·미전송 입력 복구 경로는 유지합니다.
...
- 관리 surface는 Escape 또는 바깥 pointer 입력으로 닫힌 뒤 톱니 버튼으로 focus를 돌려보냅니다. 파괴적 행동은 별도 confirmation dialog와 내부 focus 관리를 거칩니다.
- 툴팁, `더보기`, 관리 메뉴는 Escape 또는 바깥 pointer 입력으로 닫히고 소유 trigger로 focus를 돌려보냅니다. 툴팁과 `더보기`는 동시에 열리지 않으며 `prefers-reduced-motion`에서는 전환 효과를 제거합니다.
...
- 767px 이하는 하단 sheet, 768px 이상은 중앙 modal로 같은 편집 계약을 제공합니다.
- 오류가 있으면 해당 필드 가까이에서 수정 방법을 알려줍니다.
- 열린 surface 안에서 focus를 관리하고 닫힌 뒤 진입 컨트롤로 돌려보냅니다.
