...
- Main·Portfolio 설정 진행 막대와 편집 surface의 Anime.js 모션은 공통 product spring 프로필을 사용합니다. surface 등장·펼치기는 약한 bounce 0.12와 260ms perceived duration, 짧은 드래그 복귀는 220ms, 닫기는 bounce 0의 180ms, 금액·비율·막대·진행률은 bounce 0의 260ms 프로필을 사용합니다. 직접 드래그 중에는 손가락 위치를 보간하지 않으며, 금융 값은 목표를 넘지 않습니다. 연속 변경은 현재 표시값에서 이어지며 접근성용 값은 즉시 갱신하고 reduced-motion·초기화 실패에서는 최종 상태를 표시합니다. 하단 `미래 성장 보기`의 기존 CSS 등장 효과와 Simulation·브랜드 시작 장면은 기존 계약을 유지합니다.
...
- Main의 모바일 월 금액 편집·지출 계산·남는 돈 분배 sheet는 제목 여백에서 아래로 끌어 닫을 수 있습니다. 드래그는 기존 폐기 확인·저장 guard를 거치며 본문·입력은 계속 스크롤할 수 있습니다. 승인된 드래그는 최대 300ms 안에 sheet를 정리하고 원래 trigger로 focus를 돌려주며, reduced-motion에서는 지연 없이 닫습니다.
...
- Portfolio 공통 sheet는 Anime.js surface spring의 `opacity + translateY`로 아래에서 올라옵니다. 상단 손잡이를 아래로 끌면 sheet가 손가락을 따라 이동하며, 짧은 드래그는 return spring으로 복귀하고 기준을 넘긴 드래그는 기존 닫기·폐기 확인 경로를 실행합니다. 모션 감소·초기화 실패에서는 즉시 최종 위치를 표시합니다.
- `배분 수정`은 결과와 시각적으로 분리된 집중 화면을 엽니다. 767px 이하는 최대 92dvh 하단 sheet, 768px 이상은 중앙 modal이며 body만 스크롤하고 결과 영역은 편집 중 비활성화합니다. 대상 편집도 같은 최상위 dialog의 단계로 전환하며, 목록 행에서 대상 편집을 완료·취소하면 그 행으로, 신규 추가 완료 후에는 새 행으로, 신규 추가 취소·삭제 후에는 `+ 투자 대상 추가`로 focus를 돌려보냅니다.
...
- 편집, 설정, 결과 이미지 미리보기는 native `dialog`를 사용합니다. 767px 이하는 화면 아래에 붙는 sheet, 768px 이상은 중앙 modal입니다.
- 표면은 손잡이·제목·뒤로/닫기 header, 선택 context, 유일한 scroll body, 상태, footer를 같은 순서로 배치합니다. footer는 body를 가리지 않으며 일반 조작 영역은 44px 이상입니다.
- 모바일 편집·미리보기는 아래에서 올라오는 Anime.js surface spring을 사용합니다. 설정과 확인은 내용 높이에 맞추고, reduced motion에서는 즉시 최종 위치를 표시합니다.
