...
- **단위 정합성 (UI: 만원, 저장: 원)**: 모든 사용자 입력 및 표시는 **만원** 단위로 통일하여 규모감을 제공하되, 내부 계산 및 저장(IndexedDB/Bridge)은 **원** 단위를 유지하여 정밀도를 확보합니다.
- **패널 순서 (HCD Flow)**: 인지 부하를 줄이기 위해 **[요약(Summary)] -> [흐름(Sankey)] -> [상세 입력(Controls)] -> [미래 시뮬레이션(Projection)]** 순으로 정보를 배치합니다.
