...
- Portfolio 공통 하단 sheet는 열린 카드의 실제 높이만큼 `bottom`을 보간해 surface spring으로 올라옵니다. 고정 적용 버튼의 기준을 유지하고, 모바일 상단 손잡이·제목 여백을 아래로 끌면 sheet가 손가락을 따라 이동합니다. 짧은 드래그는 return spring으로 복귀하고, 기준을 넘긴 드래그는 기존 닫기·폐기 확인 경로를 실행합니다. 모션 감소·초기화 실패에서는 즉시 최종 위치를 표시합니다.
- `배분 수정`은 결과와 시각적으로 분리된 집중 화면을 엽니다. 768px 이하는 88dvh 안에서 body만 스크롤하는 하단 sheet, 769px 이상은 우측 panel이며 결과 영역은 편집 중 비활성화합니다. 대상 편집도 같은 breakpoint를 사용하고 최상위 dialog만 focus를 소유합니다. 목록 행에서 대상 편집을 완료·취소하면 그 행으로, 신규 추가 완료 후에는 새 행으로, 신규 추가 취소·삭제 후에는 `+ 투자 대상 추가`로 focus를 돌려보냅니다.
