...
- 배분 단계와 `배분 수정`은 기준 투자금, 성장·안정·현금 요약, 대상 요약 목록, 현금, `+ 투자 대상 추가`, 적용 footer 순서로 전체 배분을 먼저 읽게 합니다. 자유 편집에서는 사용자가 원화 금액을 입력하고 비율은 전체 투자금 기준으로 자동 계산합니다. 대상 행 또는 추가 action은 하나의 대상만 편집하는 공통 surface를 열며, 분류는 이름 기반 자동 추천을 기본으로 하되 두 선택지가 동시에 보이는 성장·안정 `SegmentedControl`로 바꾸면 사용자 지정이 자동 추천보다 우선합니다. 신규 추가에만 `S&P 500`, `나스닥`, `코스피`, `미국 국채`, `금 현물` 빠른 이름 선택을 보이고, 선택은 이름을 채운 뒤 금액 field로 focus를 옮깁니다. 자동 분류 상태에서는 선택한 이름으로 추천을 갱신하고 사용자 지정 분류는 보존합니다. 금액 빠른 조정은 추가·수정에 모두 제공합니다. 대상 surface에는 이름·금액 `원`·계산 비율·분류 출처와 `취소 / 완료`를 두며, 변경 후 backdrop·Escape·취소에는 폐기 확인을 요구합니다. 최초 설정의 검토와 재편집 적용 확인은 성장·안정 합계와 각 대상·현금의 금액 및 계산된 비율을 함께 보여줍니다.
...
- 배분 편집의 닫기·취소·Escape·바깥 클릭과 모바일 sheet 상단 드래그는 같은 종료 경로를 사용합니다. 변경이 없으면 닫고, 초안·미완료 현금 입력·선택한 샘플 변경이 있으면 `계속 수정 / 변경 버리기` 확인을 제공합니다. 패널 안 여백 클릭은 닫기가 아닙니다. 저장 중에는 종료를 막습니다.
- 샘플 탐색은 제목·뒤로가기를 한 곳에 두고 1100px 이상에서 목록·상세를 나란히, 그보다 좁으면 목록 → 상세로 보여줍니다. 768px 이하 샘플 탐색은 전체 높이 화면이며 일반 배분 편집 sheet와 구분합니다. 목록·상세 본문을 스크롤하고 하단 `이 구성으로 초안 채우기`는 항상 접근 가능하게 유지합니다. 선택한 비율 그대로 초안을 만들고 기존 초안 교체는 같은 화면에서 확인합니다. 상세 Escape는 목록으로, 목록 Escape는 배분 편집으로 돌아갑니다. [결과·샘플 탐색 설계](docs/superpowers/specs/2026-09-16-portfolio-overview-and-sample-navigation-design.md)를 따릅니다.
