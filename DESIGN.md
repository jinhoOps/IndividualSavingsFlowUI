...
이 문서는 Main, Simulation, Portfolio와 Account Map에 적용되는 현재 UI 계약입니다. 과거 레거시 화면의 모양이나 상호작용은 새 UI의 기준이 아닙니다.

## Experience Principles

1. **Discoverability**: AppHeader와 명확한 섹션 제목으로 현재 위치와 다음 행동을 보여줍니다.
...
- 음수, 초과 배분과 비정상 값은 검증 없이 저장하지 않습니다.
...
- 현재 목적지를 분명히 표시합니다.
- 데이터 상태는 실제 앱 상태와 연결된 경우에만 표시합니다.
...
- 주요 touch target은 쉽게 누를 수 있는 크기와 간격을 가져야 합니다.
...
- Sankey와 Account Map은 의미를 잃도록 과도하게 축소하지 않고 필요한 경우 안내가 있는 내부 스크롤을 사용합니다.
- Account Map의 compact summary는 모바일 첫 화면에서 관계도를 밀어내지 않아야 합니다.
...
- 현재 제품의 데이터 소유권과 명시적 import 경계를 유지합니다.
