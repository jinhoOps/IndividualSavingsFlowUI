...
- 적용된 결과 목록 끝에는 배경·윤곽 없는 44px 이상의 `저장하기`와 `공유하기`를 둡니다. 두 action은 같은 3:4 PNG 미리보기를 열며, 저장은 로컬 다운로드만 하고 공유는 사용자가 `공유 링크 만들기`를 명시적으로 누른 뒤에만 확인한 PNG를 올립니다. 공유 미리보기에는 링크 보유자 공개 범위와 `공유 링크는 생성 후 2일 뒤에 만료돼요.`를 표시합니다. 금액 제외 모드는 원화 수치를 출력 모델에서 제거하며, 링크 수신 화면은 로그인·편집·workspace 조회 없이 이미지와 만료 안내만 표시합니다. 완료한 공유는 이후 계획 수정으로 바뀌거나 만료가 연장되지 않습니다.
