...
- 저장 전 확인 단계에서는 투자 대상 수와 성장·안정 구성을 요약하고, 편집 중 변경과 저장 결과를 명확히 구분합니다.
- 투자금 0원은 기존 계획을 보존하고 Main 투자금 편집으로 안내합니다.
- 현재 배분 편집과 결과는 항상 `전체 기준`이 우선입니다.
- 배분 결과·편집 다음에 `투자 위치`를 두고 공유 금융 위치의 생성, 이름 변경, 보관을 제공합니다.
- 투자 위치는 최대 8자 이름, 형태와 선택 기관을 보여주며 빈 위치는 `아직 배분하지 않음`으로 명시합니다.
- Phase A의 위치 상태 action은 44px touch target을 유지하고 위치별 배분을 편집할 수 있는 것처럼 표현하지 않습니다.
...
- Portfolio 공유 투자 위치는 390px에서 이름·상태·action이 패널 밖으로 넘치지 않아야 합니다.
