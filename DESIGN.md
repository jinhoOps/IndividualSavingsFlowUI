...
- 결과는 목표 도달 핵심 문장과 조건 한 줄 뒤에 하나의 자산 변화 패널을 제공합니다. 패널은 `N년 동안의 자산 변화` 제목(0년은 `현재 자산`)과 현재 명목·실질 기준, 그래프, 낮은 강조의 비교값, 하단 중앙의 투명한 `조건 편집` 버튼 순서로 읽힙니다. 버튼은 44px 이상이며 모바일에서는 하단 시트, 768px 이상에서는 중앙 모달을 열고 닫으면 버튼으로 focus를 돌려줍니다. 그래프·조작·비교에 중첩 카드 외곽선을 만들지 않습니다. 비교 배율은 `넣은 돈 대비 N배`로 표시합니다.
