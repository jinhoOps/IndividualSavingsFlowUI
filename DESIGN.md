...
- Main·Portfolio 설정 진행 막대는 공통 Anime.js 훅으로 180ms 보간합니다. Main 지출·남는 돈 도우미는 8px·180ms로 등장하고, 적용된 월 자금 배분 막대·수입 기준선·요약 비율은 260ms로 함께 전환합니다. 연속 변경은 현재 표시값에서 이어지며 접근성용 값은 즉시 갱신하고 reduced-motion·초기화 실패에서는 최종 상태를 표시합니다. 하단 `미래 성장 보기`의 기존 CSS 등장 효과는 예외로 유지합니다.
...
- Portfolio 공통 하단 sheet는 열린 카드의 실제 높이만큼 화면 아래에서 Anime.js로 260ms 동안 올라옵니다. 고정 적용 버튼의 기준을 유지하도록 `bottom`을 보간하고, 모션 감소·초기화 실패에서는 즉시 최종 위치를 표시합니다.
...
- 배분 편집의 닫기·취소·Escape·바깥 클릭은 같은 종료 경로를 사용합니다. 변경이 없으면 닫고, 초안·미완료 현금 입력·선택한 샘플 변경이 있으면 `계속 수정 / 변경 버리기` 확인을 제공합니다. 패널 안 여백 클릭은 닫기가 아닙니다. 저장 중에는 종료를 막습니다.
