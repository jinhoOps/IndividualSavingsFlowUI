...
대상 행이나 추가 action은 같은 `PortfolioItemSheet`를 연다. 이 surface는 이름, 금액 `원`, 계산 비율, 성장/안정 선택과 분류 출처, footer 순서다. 성장/안정은 이름 오른쪽의 단일 toggle이 아니라 두 선택지가 동시에 보이는 `SegmentedControl`이다. 사용자가 선택하면 분류 출처는 사용자 지정이 되고, 기존 `자동 추천 사용` 경로는 유지한다. 빠른 이름 선택은 신규 추가에서만, `-50만 / -10만 / +10만 / +50만` 금액 빠른 조정은 추가·수정 모두에서 제공한다.
