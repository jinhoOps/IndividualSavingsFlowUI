...
기존 setup의 1/6 welcome content와 6/6 돈 조립 막대는 Anime reveal이 시작되기 전 각각 투명·`scaleX(0)` 상태로 설정된다. 현재는 animation 생성의 동기 실패만 처리하고, 생성 뒤 timeline이 진행되지 않는 경우 final state로 복구하지 않는다. 이 경우 사용자는 1/6에서 본문 reveal을, 6/6에서 실제 돈 조립 막대를 보지 못하고 상단 초록 progress strip만 보게 된다.
...
- normal motion에서 기존 welcome reveal과 review assembly timing·순서를 유지하되, setup 진행 action은 reveal 대상에서 제외해 빠른 단계 전환 중에도 항상 시각적으로 보인다.
- 1/6 welcome reveal의 제목·설명과 6/6 assembly의 heading·segment·돈 조립 막대는 각 animation의 최대 예정 시간 뒤에도 final state에 도달하지 못하면 final styles로 복구한다.
...
`SetupFlow`의 motion scope는 animation 시작 직후 final-state deadline을 등록한다. normal animation completion은 deadline을 취소하며, deadline이 먼저 오면 현재 scope가 소유한 reveal/assembly elements에 기존 final-style helper를 적용한다. scope cleanup은 deadline을 해제하므로 strict-mode replay·step change·unmount가 stale completion을 만들지 않는다.
...
- unit: timeline 생성은 성공하지만 completion/animation tick이 오지 않는 fixture에서 welcome action은 즉시 visible state를 유지하고 review track·segments·content가 deadline 뒤 final styles가 된다. step change와 unmount 뒤 stale deadline은 적용되지 않는다.
- browser: normal real-time 1/6과 filled-data 6/6에서 예정 시간 뒤 action, assembly track, segments와 content가 final state인지 확인한다. 가상 clock만으로 이 계약을 증명하지 않는다.
