...
기존 setup의 1/6 `다음`과 6/6 돈 조립 막대는 Anime reveal이 시작되기 전 각각 투명·`scaleX(0)` 상태로 설정된다. 현재는 animation 생성의 동기 실패만 처리하고, 생성 뒤 timeline이 진행되지 않는 경우 final state로 복구하지 않는다. 이 경우 사용자는 1/6에서 진행 action을, 6/6에서 실제 돈 조립 막대를 보지 못하고 상단 초록 progress strip만 보게 된다.
...
- normal motion에서 기존 welcome reveal과 review assembly timing·순서를 유지한다.
- 1/6 welcome reveal의 제목·설명·`다음`과 6/6 assembly의 heading·segment·돈 조립 막대는 각 animation의 최대 예정 시간 뒤에도 final state에 도달하지 못하면 final styles로 복구한다.
...
- unit: timeline 생성은 성공하지만 completion/animation tick이 오지 않는 fixture에서 welcome `다음`과 review track·segments·content가 deadline 뒤 final styles가 된다. step change와 unmount 뒤 stale deadline은 적용되지 않는다.
- browser: normal real-time 1/6과 filled-data 6/6에서 예정 시간 뒤 `다음`, assembly track, segments와 content가 final state인지 확인한다. 가상 clock만으로 이 계약을 증명하지 않는다.
