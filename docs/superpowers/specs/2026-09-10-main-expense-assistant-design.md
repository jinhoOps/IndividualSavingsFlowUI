...
관리비는 공용관리비(일반관리비), 수도세, 전기세, 가스비를 포함한다고 설명한다. 관리비에 포함된 요금은 합치고 따로 내는 요금만 뒤의 공과금에서 답하도록 안내하여 중복 합산을 피한다. 월세와 대출 이자는 각각 답할 수 있다. 목적별로 모든 연간 환산 금액을 더한 뒤 12로 나누어 원 단위 반올림을 한 번만 한다. TypeScript BigInt와 SQL numeric을 사용하며 음수·소수 원금액·안전 정수 초과·합계 범위 초과는 거부한다.
