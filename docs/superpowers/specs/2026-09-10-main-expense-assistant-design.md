...
처음에는 다섯 월 금액을 대략 입력한다. 결과의 **지출 금액**을 누르면 지출 계산 도우미가 열린다. 2026-09-15 사용자 요청에 따라 금액 행과 도우미 제목의 지팡이 아이콘은 제거한다. 금액 아래 `항목별로 계산`을 표시하고, 명칭·비율 영역과 도넛은 항목 탐색을 유지한다. 화면 문구는 `지출`로 통일하며 내부 consumption 계산 키는 유지한다.
