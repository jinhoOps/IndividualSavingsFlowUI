...
- 첫 투자 대상을 추가하는 단일 primary action을 우선한다.
- 입력 방식 기본값은 현재 계약대로 `금액`이다. `비율` 전환은 같은 편집 surface 안의 보조 control로 유지한다.
- 현금은 남은 투자금 자동 배분 상태로 설명하며 별도 투자 대상처럼 먼저 설정하도록 요구하지 않는다.
- 마지막 검토에서 투자 대상 수, 총 투자금, 현금 비중을 확인하고 `배분 시작`으로 적용한다.
- 최초 설정 draft는 현재 Portfolio draft 저장 계약을 그대로 사용하며 새로고침 후 재개한다.
...
- 새로고침 또는 앱 이탈은 기존 계약대로 draft를 보존하며 재진입 시 수정 surface로 복원한다.

## 적용 확인

현재 확인 dialog의 정보는 짧은 요약 행으로 재구성한다.
...
- 수정은 모바일과 같은 bottom sheet 계약을 사용한다.
...
- 최초 설정 새로고침은 기존 draft와 단계에 맞는 편집 상태를 복원한다.
...
- 최초 설정, 결과와 수정이 서로 다른 시각·상호작용 상태로 인지된다.
