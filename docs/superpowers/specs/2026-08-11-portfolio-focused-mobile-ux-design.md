...
- 마지막 검토에서 투자 대상 수, 총 투자금, 현금 비중을 확인하고 `배분 시작`으로 즉시 적용한다. 최초 설정에서는 별도 적용 확인 dialog를 다시 열지 않는다.
...
2. `배분`: 투자 대상과 금액 또는 비율을 편집한다.
3. `검토`: 투자 대상 수, 투자금, 현금 비중과 결과 요약을 확인한다.
...
- 최초 설정 검토의 `배분 시작`은 별도 확인 dialog 없이 적용하며, 수정 적용만 확인 dialog를 사용한다.
