...
- `배분` 단계는 월 투자금과 성장·안정 비율의 라이브 요약을 입력보다 먼저 보여준다. 완성된 투자 대상은 이름·분류·금액·비율을 한 줄로 요약하고 사용자가 해당 행을 선택할 때만 편집 control을 펼친다. 새 대상은 즉시 열린 상태로 시작한다. 현금도 금액·비율과 자동 배분 상태를 한 줄로 요약하고 명시적 선택 때만 직접 편집을 펼친다.
