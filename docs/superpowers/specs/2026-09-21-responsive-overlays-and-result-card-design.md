...
| 모바일 편집 | Main 월 금액처럼 Simulation도 아래에서 올라온다 | `조건 편집` 하단 바 → 하단 시트; 웹에서는 같은 내용의 중앙 모달 |
...
| 결과 보관 | 고정지출부터 배분까지 입력한 결과를 3:4 이미지로 예쁘게 보관한다 | Portfolio 결과 아래 `계획 이미지 저장` → 미리보기 → PNG 저장/기기 공유 |
...
- [MDN Web Share](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/share): 파일 공유는 지원 여부와 사용자 활성화가 필요하다. PNG를 미리 생성하고 명시적 공유 버튼에서 호출하며 다운로드 fallback을 제공한다.
...
결과 화면: 목표 도달 요약 → 자산 변화와 명목/실질 선택 → 비교 결과 → 조건 요약/편집 → 하단 Portfolio 진입 순서다.

- 모바일에 Main과 같은 하단 `조건 편집` 바를 제공한다. bar는 tap 진입점이며 bar 자체를 드래그해 입력을 바꾸지 않는다.
- desktop에서는 결과 영역 하단 오른쪽 `조건 편집` 버튼으로 중앙 모달을 연다.
- 편집 내용은 `기간·기대수익률` → `시작 자산·목표` → `기준금리·물가` 순서다. 기존 0~30년, 5/9/13%, 직접 입력/증감, 목표 검증과 기본 목표 복원을 유지한다. 중첩 accordion은 만들지 않는다.
...
- 명목/실질은 결과 화면에서 유지하며 현재의 저장 경로를 사용한다. 편집기 열린 동안 배경 조작은 차단된다. 최초 onboarding은 페이지 흐름으로 유지한다.
...
- 모바일 `조건 편집` 바와 다음 단계가 겹치지 않게 하단 여백과 노출 후 스크롤 위치를 계산한다. sheet/설정/그래프 조작/내부 스크롤 중에는 reveal하지 않는다.
...
새 `계획 이미지 저장`은 적용된 결과 아래에 둔다. 톱니 메뉴 깊숙이 숨기지 않는다. 첫 배분 적용 뒤에도 같은 위치에 자연스럽게 도달하며 저장 UI를 강제로 띄우지 않는다.
...
1. `계획 이미지 저장`을 누르면 웹 모달/모바일 큰 시트에 실제 생성할 이미지와 옵션을 보여준다.
...
5. 고정 PNG를 브라우저에서 생성한다. `PNG 저장`은 다운로드 요청이며 `기기에 사진 저장 완료`로 단정하지 않는다. 모바일은 파일 공유가 가능할 때 `공유`를 제공하고 운영체제의 공유 시트를 연다. 공유 취소는 실패 경고를 띄우지 않는다.
6. 생성→준비→다운로드/공유 상태를 구분한다. 이미지 Blob을 준비한 뒤 명시적인 공유 클릭에서 `navigator.share`를 호출한다. 미지원·실패에서는 PNG 다운로드를 유지한다.
7. 외부 렌더 서비스·이미지 업로드·Supabase bucket을 만들지 않는다. browser 생성·download이며 재다운로드에 재계산할 필요가 없다. object URL과 메모리는 닫기/교체 때 해제한다.
...
- DB migration, backtest, 시세 수집, 수익률 보장, 새 계좌 UI, 공개 공유 페이지는 범위 밖이다.
