...
**Spec:** [기존 공통 표면 설계](../specs/2026-09-22-unified-bottom-sheet-design.md), [DESIGN](../../../DESIGN.md), [PRD](../../ways-of-work/plan/isf-rebuild/connected-financial-planning-workspace/prd.md). 아래 확정 요구사항은 2026-09-23 사용자 수정 사항이며 기존 설계의 92dvh·손잡이 전용 드래그·공통 모션을 대체한다. 이 문서는 **구현 전 계획**이다. 제품 코드와 기존 문서의 완료 상태를 먼저 변경하지 않는다.
...
- `responsive-dialog.css`: 손잡이가 웹에도 보이고 제목이 중앙 정렬된다. form/wide에 각각 46rem/54rem 고정 높이가 적용된다. 모바일 상한은 92dvh다.
- `useSheetDismiss.ts`: 손잡이에만 pointer listener가 붙고, 복귀·닫기 fallback은 각각 300ms다. 이 타이머를 두고 모션 설정만 늘리면 중간에 끊긴다.
- `ResponsiveDialog.tsx`: 모바일 등장만 opt-in이고 웹은 즉시 나타난다. 버튼/Escape/backdrop 종료는 승인 후 즉시 finishClose하며, 드래그만 별도 퇴장 모션이 있다.
- `useSheetDismiss.test.tsx`: 본문에서는 드래그하지 않는다는 기존 테스트가 있어 새로운 스크롤 우선 계약으로 교체해야 한다.
- 기존 설계 문서의 modal 너비와 현재 CSS 수치가 다르므로 이번 구현에서 아래 수치로 코드·문서를 맞춘다.
...
- [ ] 1. `tests/simulation.spec.ts`에 `surface geometry` 그룹을 추가한다. 기존 `seedMain`·`openFirstResult`로 편집기를 열고 아래 경계를 실패하는 테스트로 고정한다.
...
- [ ] 2. `npx playwright test tests/simulation.spec.ts --grep 'surface geometry'` 실행. 현재 92dvh/웹 손잡이 때문에 실패하는지 확인한다.
- [ ] 3. CSS를 변경한다. 모바일 손잡이 표시선은 `clamp(72px, 22vw, 104px)`·높이 4px, 실제 손잡이 행은 가로 전체·높이 44px 이상이다. 좁은 40px 요소만 잡도록 하지 않는다. 제목·손잡이·본문 사이의 불필요한 높이는 줄인다.
...
.responsive-dialog[data-presentation='modal'] { height: auto; }
...
```
  ...
  웹 header에만 빈 왼쪽 spacer를 제거하고, 뒤로가기가 있는 경우 실제 버튼 열은 유지한다. 웹 좌우 여백 24px·모바일 20px를 기준으로 한다. fixed height와 자식 `height:100%`를 함께 제거하고 max-height 전달·body `min-height:0`를 유지한다. 모바일 full만 88dvh로 채운다. 짧은 화면 예외도 88dvh·footer 접근 기준으로 다시 맞춘다.
- [ ] 4. 테스트를 통과시키고 긴 본문과 footer 없는 설정 화면에서도 내부 스크롤만 생기는지 확인한다.
- [ ] 5. `git diff --check`와 작성자 확인 후 관련 파일만 커밋: `feat: separate sheet and modal presentation`.
...
- [ ] 1. 390×844, 390×600, 320×568, 768×1024, 1280×900에서 편집·설정·확인·미리보기를 순회한다. sheet 88% 상한, modal 중앙 정렬, 전체 가로 overflow 없음, footer와 focused input 가시성, 44px 버튼을 확인한다. 200% 확대와 긴 Portfolio 이름/오류도 포함한다.
- [ ] 2. `requestAnimationFrame`으로 transform/opacity를 기록해 초기·중간·종료 프레임을 검사한다. 진입/닫기/복귀의 animation duration=450ms와 정상 환경의 완료 400–500ms를 확인한다. 고정 sleep 후 마지막 화면만 검사하지 않는다. 부하로 프레임이 늦어졌다면 측정값을 남기고 코드상 duration과 구분한다.
- [ ] 3. 실제 브라우저 터치로 본문에서 위·아래 스크롤, 맨 위 새 아래 드래그, 긴 손잡이 양 끝 드래그, 입력·토글·slider 조작, 드래그 취소를 수행한다. 웹에서는 같은 여백 드래그로 위치나 종료가 바뀌지 않는지 확인한다.
- [ ] 4. 변경 후 드래그→폐기 확인→계속 수정/버리기, 저장 중 종료 차단, 깨끗한 상태 즉시 승인, 중첩 확인 focus trap/복원을 검증한다. 진입·복귀 중 다시 조작, 열린 상태에서 767↔768px 전환·viewport 높이 축소도 확인한다.
- [ ] 5. `npm run check:ci` 및 `npx playwright test` 실행. 여러 앱이 쓰는 공통 인프라 변경이므로 전체 E2E가 필요하다. 실패는 재현·원인·수정·재검증을 기록하고 단일 전체 실행과 부분 재실행 결과를 구분한다.
- [ ] 6. 검증 기록에는 명령·성공/실패 수·모션 측정값·화면 캡처·실기기 미검증 범위를 적는다. 코드/테스트를 커밋: `test: cover sheet gestures and modal presentation`.

## Task 6: 문서 정합성·리뷰·푸시

**Files:** `DESIGN.md`, 위 기존 공통 표면 spec, PRD의 공통 surface 항목, 이 계획과 evidence.

- [ ] 1. `DESIGN.md`에 남은 Main side panel/우측 panel 문구, 92dvh, 손잡이 전용 드래그, 즉시 표시 웹 모달 계약을 새 규격으로 갱신한다. 제품 소유권과 저장 계약은 바꾸지 않는다.
- [ ] 2. 기존 spec에서 실제 선택한 compact=440/form=640/wide=1080px, 88dvh, 손잡이, 스크롤 우선, 총 450ms와 fallback의 차이를 명시한다. PRD는 두 표면 규격과 상세 spec 링크만 유지해 중복하지 않는다.
- [ ] 3. 편집 문서의 상대 링크, 상태 주장, `git diff --check`를 검증한다. 계획 체크박스는 실제 완료된 작업만 체크한다.
- [ ] 4. 전체 diff의 공통 종료·터치·focus 회귀 리뷰를 수행한다. 승인 없는 모델 변경이나 별도 다중 작업 실행을 추가하지 않는다. 발견 사항은 수정 후 해당 검증을 다시 실행한다.
...
- [ ] 모든 모바일 표면이 88dvh 이하이고 손잡이·비상호작용 공간에서 정상 드래그된다.
- [ ] 본문 스크롤·입력·토글 조작이 닫기로 오인되지 않는다.
- [ ] 웹은 손잡이 없는 내용 높이 중앙 모달과 공통 행동 정렬을 사용한다.
- [ ] 모달에 수직 등장 이동이 없고 전체 모션은 잔여까지 400–500ms 안에 종료된다.
- [ ] 세 앱의 저장·단계·오류·focus 계약과 reduced-motion을 보존한다.
