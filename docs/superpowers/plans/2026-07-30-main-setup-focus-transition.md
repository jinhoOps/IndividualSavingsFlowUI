...
- 이동 완료 후 단색 레이어만 300ms 동안 opacity 1→0으로 전환해 아래의 최종 색상 바를 드러낸다.
- 6/6 초기에는 allocation 카드 윤곽·설명·표·색상 바를 시각적으로 숨기고, 색상 전환 종료 후 카드 윤곽·설명·표를 250ms 동안 표시한다.
- 설정 카드의 네 금액 조정 버튼은 390px에서도 한 줄인 4열 균등 그리드와 최소 44px 높이를 유지한다.
