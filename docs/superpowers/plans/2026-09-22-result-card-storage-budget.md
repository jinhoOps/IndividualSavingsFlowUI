...
- [x] 로컬 설정을 만들고 `supabase start`, `supabase functions serve --env-file supabase/.env.local`로 시험 서버를 연다. `.env.local`은 Git에서 제외하고 로컬 CLI가 발급한 값과 로컬 전용 cleanup secret만 사용한다. config의 두 함수는 `verify_jwt=false`로 두되 POST 내부 인증과 cleanup secret 검증은 유지한다. 시험 서버를 구성하고 검증했다. 종료 확인은 아래 별도 항목을 따른다.
...
- [ ] 로컬 시험 컨테이너 종료를 확인한다. `supabase stop --project-id result-card-storage-budget --no-backup` 실행 중 OrbStack Docker API가 응답하지 않았다. 시험 CLI 프로세스·Orca 하위 작업공간·임시 자격 증명은 정리했으며, 컨테이너와 volume 제거는 확인되지 않았다. 다른 프로젝트를 중단하는 `--all`이나 OrbStack 전체 재시작은 수행하지 않았다.
