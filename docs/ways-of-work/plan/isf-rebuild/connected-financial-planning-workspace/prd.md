...
현재 지원 제품은 **Main, Simulation, Portfolio와 Account Map**이다. Main은 월 자금 흐름을, Simulation은 장기 복리를, Portfolio는 최신 Main 투자금의 전체 기준 배분을 보여준다. Account Map은 Main 기준의 목적 배정과 계좌 간 월 계획 흐름을 보여준다. 네 앱은 workspace schema v4의 단일 `isf-workspace-v4` 기록을 사용한다.
...
- Main, Simulation, Portfolio, 공유 금융 위치와 Account Map applied/draft를 schema v4의 하나의 `isf-workspace-v4` 문서에 저장한다.
...
현재 제품의 저장 boundary는 workspace schema v4의 `isf-workspace-v4` 하나다. 여기에는 Main applied/setup progress, Simulation draft, aggregate-only Portfolio plans/draft, 공유 금융 위치와 Account Map applied/draft(목적 link와 account transfer 포함)가 들어간다. current Account Map state에는 `legacyPhaseA`나 `layout`이 없다. 유효한 v3 `isf-workspace-v3`은 v4가 없을 때만 read-only conversion/rollback source이고, retired v1/v2 `isf-workspace-v1`은 v4와 v3가 모두 없을 때만 같은 후보가 된다. 성공한 conversion도 원본을 변경하거나 삭제하지 않는다.
