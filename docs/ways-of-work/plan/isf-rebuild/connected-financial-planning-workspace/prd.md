...
2026-09-11 승인: 로그인 후 실제 workspace 로딩 동안 브랜드 모션을 한 번 재생한다. 데이터 준비가 끝나면 즉시 진입하며, 지연 시 완성 프레임과 로딩 문구를 유지한다. 기존 세션의 앱 이동·새로고침은 정적 로고, reduced motion도 정적 로고를 사용한다. Main의 새 시작·다시 시작·초기화는 별도 인트로 없이 setup으로 진입한다. [계정 로딩 설계](../../../../superpowers/specs/2026-09-11-account-loading-brand-motion-design.md)를 따른다.
