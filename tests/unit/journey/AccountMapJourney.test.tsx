...
describe('AccountMapJourney', () => {
  it('closes an open Main overlay when the account workspace loses Main', async () => {
    const setup = repositoriesWithStaleMain();
    const { rerender } = render(<AccountMapJourney repositories={setup.repositories} mainRepository={setup.mainRepository} mainAvailable />);
    fireEvent.click(screen.getByRole('button', { name: 'Main 금액 수정' }));
    await screen.findByRole('dialog', { name: '월 자금 계획 편집' });
    await waitFor(() => expect(screen.getByTestId('account-map-journey-background')).toHaveAttribute('inert'));

    rerender(<AccountMapJourney repositories={setup.repositories} mainRepository={setup.mainRepository} mainAvailable={false} />);

    await waitFor(() => expect(screen.queryByRole('dialog', { name: '월 자금 계획 편집' })).not.toBeInTheDocument());
    expect(screen.getByTestId('account-map-journey-background')).not.toHaveAttribute('inert');
  });
