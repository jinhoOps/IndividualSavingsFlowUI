...
import { CustomPurposeDialog } from '../../../src/account-map/ui/AccountMapSetup';
...
describe('AccountMapSetup', () => {
  it('restores the v4 custom-purpose form without saving and clears it on explicit cancel', async () => {
    const setup = repositories();
    const workspace = setup.current();
    const recordRecoveryDraft = vi.fn();
    const session = {
      readRecoveryDraft: (key: string) => key === 'account-map-custom-purpose'
        ? { parentId: 'system:saving', name: '여행', amountWon: 120_000 }
        : null,
      recordRecoveryDraft,
    ...
    render(
      <AccountDraftContext.Provider value={session}>
        <CustomPurposeDialog
          ...
          draft={{
            schemaVersion: 2, sourceMainUpdatedAt: workspace.main.applied!.updatedAt,
            customPurposes: [], links: [], transfers: [], step: 'locations', updatedAt: 1,
          }}
          disabled={false}
          onCancel={vi.fn()}
          onSave={onSave}
    ...
    expect(screen.getByRole('textbox', { name: '월 금액' })).toHaveValue('120,000');
    expect(onSave).not.toHaveBeenCalled();

    fireEvent.click(screen.getByRole('button', { name: '취소' }));
    await waitFor(() => expect(recordRecoveryDraft).toHaveBeenCalledWith('account-map-custom-purpose', null));
  });

  it('scopes unsent location-picker input to the selected purpose', () => {
    const setup = repositories('locations');
    const session = {
      readRecoveryDraft: (key: string) => key === 'account-map-picker:system:income'
        ? { mode: 'create', selectedLocationId: null, locationFields: { kind: 'brokerage', institution: { name: '미래증권' }, shortName: '투자계좌' }, amountWon: 0 }
        : null,
      recordRecoveryDraft: vi.fn(),
    } as unknown as AccountWorkspaceSession;

    render(<AccountDraftContext.Provider value={session}><AccountMapApp repositories={setup.repositories} /></AccountDraftContext.Provider>);
  ...
  it('does not carry one purpose picker state into another purpose recovery key', () => {
    const setup = repositories('locations');
    const session = {
      readRecoveryDraft: (key: string) => key === 'account-map-picker:system:income'
        ? { mode: 'create', selectedLocationId: null, locationFields: { kind: 'brokerage', institution: { name: '수입증권' }, shortName: '수입복구' }, amountWon: 0 }
        : key === 'account-map-picker:system:living'
          ? { mode: 'create', selectedLocationId: null, locationFields: { kind: 'bank', institution: { id: 'hana', name: '하나은행' }, shortName: '생활복구' }, amountWon: 0 }
          : null,
      recordRecoveryDraft: vi.fn(),
    } as unknown as AccountWorkspaceSession;
    ...
    expect(session.recordRecoveryDraft).not.toHaveBeenCalledWith(
      ...
      expect.objectContaining({ locationFields: expect.objectContaining({ shortName: '수입복구' }) }),
    );
  });

  it('scopes unsent transfer input to the setup add editor', () => {
    const setup = repositories('transfers');
    const session = {
      readRecoveryDraft: (key: string) => key === 'account-map-transfer:setup:add'
        ? { sourceLocationId: 'salary', targetLocationId: 'brokerage', allocationKind: 'sweep', monthlyAmountWon: 0, status: 'active' }
        : null,
      recordRecoveryDraft: vi.fn(),
    } as unknown as AccountWorkspaceSession;
...
function repositories(
  step: 'basis' | 'locations' | 'transfers' | 'review' = 'basis',
  ...
  if (step === 'locations' || step === 'transfers') {
    workspace.locations = [
