...
describe("AccountMapModal", () => {
  it("restores a scoped v4 node edit without saving and clears it on cancel", async () => {
    const recordRecoveryDraft = vi.fn();
    const session = {
      readRecoveryDraft: (key: string) => key === "account-map-node:location:checking" ? {
        editLabel: "복구 이름",
        locationFields: { kind: "brokerage", institution: { name: "미래증권" }, shortName: "복구 계좌" },
        editTarget: "",
        editLinks: [{ id: "living-link", monthlyAmountWon: "650000", status: "active", remainder: true }],
    ...
    render(<AccountDraftContext.Provider value={session}><AccountMapModal {...modalProps({
      initialMode: "edit",
      node: { id: "location:checking", kind: "location", label: "생활비통장", status: "resolved" },
      locations: [{ id: "checking", shortName: "생활비통장", kind: "bank", institution: { id: "hana", name: "하나은행" }, roles: ["spending"], createdAt: 1, updatedAt: 1 }],
      related: [{ label: "생활비", amountWon: 700_000, status: "active", linkId: "living-link", purposeId: "system:living", remainder: true, relationKind: "purpose-link" }],
      onSaveEdit,
    ...
    expect(screen.getByRole("textbox", { name: "표시 이름" })).toHaveValue("복구 계좌");
    expect(screen.getByRole("textbox", { name: "생활비 월 금액" })).toHaveValue("650,000");
  ...
  it("does not recreate a dirty node recovery record while the modal is closing", async () => {
    const recordRecoveryDraft = vi.fn();
    const session = {
      readRecoveryDraft: () => ({
        editLabel: "복구 생활비",
        locationFields: { kind: "bank", shortName: "생활비" },
        editTarget: "900000",
        editLinks: [],
        editReplacementByPurpose: {},
      }),
    ...
    render(<AccountDraftContext.Provider value={session}><AccountMapModal {...modalProps({
      initialMode: "edit",
      node: { id: "custom:living-extra", kind: "purpose", label: "추가 생활비", amountWon: 800_000, status: "resolved" },
    })} /></AccountDraftContext.Provider>);
  ...
  it("uses one compact connection action and keeps connect content in the same dialog", () => {
    ...
    expect(screen.getByRole("dialog", { name: "생활비 연결 추가" })).toBe(
      dialog,
    );
  ...
  it("offers purpose archive only from a custom-purpose title menu", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "생활비 더보기" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("menuitem", { name: "목적 보관" }),
    ).not.toBeInTheDocument();
    system.unmount();
    ...
    expect(
      screen.queryByRole("button", { name: "생활비통장 더보기" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("menuitem", { name: "목적 보관" }),
    ).not.toBeInTheDocument();
  ...
  it("confirms custom-purpose archive inside the same dialog shell", async () => {
    ...
    expect(dialog).toContainElement(document.activeElement as HTMLElement);
    expect(
      screen.getByText("생활비통장 100,000원 연결이 중지됩니다"),
    ).toBeVisible();
  ...
  it("locks custom-purpose archive while recovery is active", () => {
    renderModal({
      ...
      recovery: {
        status: "manual",
        latest: createEmptyWorkspace(2),
        action: "edit-node",
        targets: [{ kind: "node", id: "custom:telecom" }],
        reason: "compound-edit",
      },
  ...
  it("lists every active unlinked location without filtering by its current roles", () => {
    ...
    expect(
      screen.queryByRole("button", { name: /생활비통장/ }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /예전통장/ }),
    ).not.toBeInTheDocument();
  });
  ...
  it("creates a brokerage location from direct institution input in connect mode", () => {
    const onCreateAndConnectLocation = vi.fn<
      NonNullable<
        React.ComponentProps<
          typeof AccountMapModal
  ...
  it("creates a cash location without institution in connect mode", () => {
    ...
    expect(onCreateAndConnectLocation.mock.calls[0]?.[0]).not.toHaveProperty(
      "institution",
    );
  });
  ...
  it("edits a location kind, institution, and name as one detail payload", () => {
  ...
  it("requires location detail and link changes to be saved separately", () => {
    renderModal({
      node: {
        id: "location:checking",
        kind: "location",
    ...
    fireEvent.change(
      screen.getByRole("textbox", { name: "생활비 월 금액" }),
      { target: { value: "650000" } },
    );

    expect(screen.getByText("계좌 정보와 연결은 따로 저장해 주세요.")).toBeVisible();
  ...
  it("submits edited link amounts instead of exposing a no-op save", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "편집" }));
    fireEvent.change(
      screen.getByRole("textbox", { name: "생활비통장 월 금액" }),
      { target: { value: "650000" } },
    );
    fireEvent.click(screen.getByRole("button", { name: "저장" }));
    expect(onSaveEdit).toHaveBeenCalledWith(
  ...
  it("requires and submits a replacement when editing away a location remainder", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "편집" }));
    fireEvent.change(
      screen.getByRole("combobox", { name: "생활비 연결 상태" }),
      { target: { value: "suspended" } },
    );
    expect(screen.getByRole("button", { name: "저장" })).toBeDisabled();
    fireEvent.change(
      screen.getByRole("combobox", { name: "편집 나머지 연결" }),
      { target: { value: "next" } },
    );
    fireEvent.click(screen.getByRole("button", { name: "저장" }));
    expect(onSaveEdit).toHaveBeenCalledWith(
  ...
  it("requires an excess correction before restoring selected links", () => {
    ...
    expect(
      screen.getByRole("combobox", { name: "복원 나머지 연결" }),
    ).toBeRequired();
  ...
  it("shows suspended impact and requires a replacement remainder", () => {
    ...
    expect(
      screen.getByText("생활비 700,000원 연결이 중지됩니다"),
    ).toBeVisible();
    expect(
      screen.getByRole("combobox", { name: "새 나머지 계좌" }),
    ).toBeRequired();
  });
  ...
  it("allows location-only restore and never offers user-suspended links for selection", () => {
    ...
    expect(
      screen.queryByRole("checkbox", { name: /저축/ }),
    ).not.toBeInTheDocument();
  ...
  it("keeps the archive selection and offers retry after a failed command", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "보관" }));
    fireEvent.change(screen.getByRole("combobox", { name: "새 나머지 계좌" }), {
      target: { value: "next" },
    });
    fireEvent.click(screen.getByRole("button", { name: "보관하기" }));
  ...
  it("preserves typed input and describes and focuses the first colliding modal field", () => {
    ...
    expect(
      screen.getByRole("button", { name: "최신 상태에서 다시 적용" }),
    ).toBeVisible();
  ...
  it("shows transport failure inside a recovering modal and disables recovery while pending", () => {
    ...
    fireEvent.pointerDown(
      document.querySelector(".account-map-modal-backdrop")!,
    );
  ...
  it("keeps normal removal available as an atomic edit", () => {
    ...
    expect(onSaveEdit).toHaveBeenCalledWith(
      expect.objectContaining({
        links: expect.arrayContaining([
          expect.objectContaining({ id: "living", status: "removed" }),
        ]),
      }),
    );
  });
  ...
  it("keeps compound input and offers latest review without automatic replay", () => {
    ...
    rerender(
      <AccountMapModal
        ...
        recovery={{
          ...
          latest: createEmptyWorkspace(2),
          action: "edit-node",
          targets: [{ kind: "node", id: "location:checking" }],
          reason: "compound-edit",
        }}
      />,
    );
    ...
    expect(
      screen.queryByRole("button", { name: "최신 상태에서 다시 적용" }),
    ).not.toBeInTheDocument();
  ...
  it.each(["close", "escape", "backdrop"] as const)(
    "abandons settled recovery through %s without replaying",
    (method) => {
      const onClose = vi.fn();
  ...
  it("restores focus before adopting settled recovery when reduced motion closes immediately", () => {
    ...
    const onKeepLatest = vi.fn(() =>
      sequence.push(
        document.activeElement === source ? "focus" : "adopted-before-focus",
      ),
    );
...
function renderModal(
  overrides: Partial<React.ComponentProps<typeof AccountMapModal>> = {},
) {
  const source = document.createElement("button");
  document.body.append(source);
  return render(
    <AccountMapModal
      {...modalProps({ sourceElement: source, ...overrides })}
    />,
  );
}
...
function modalProps(
  overrides: Partial<React.ComponentProps<typeof AccountMapModal>> = {},
): React.ComponentProps<typeof AccountMapModal> {
  return {
    node: {
      id: "system:living",
      kind: "purpose",
