...
describe('AccountMapApp completed flow map', () => {
  it('restores unsent input in the scoped completed-map transfer editor', () => {
    ...
    expect(within(dialog).getByRole('combobox', { name: '보내는 계좌' })).toHaveValue('salary');
    expect(within(dialog).getByRole('combobox', { name: '받는 계좌' })).toHaveValue('brokerage');
    expect(within(dialog).getByRole('radio', { name: '남은 금액 전부' })).toBeChecked();
  });

  it.each(['header close', 'Escape'])('clears a completed-map transfer recovery on explicit %s', async (action) => {
    const setup = flowRepositories();
    const recordRecoveryDraft = vi.fn();
    const session = {
      readRecoveryDraft: (key: string) => key === 'account-map-transfer:map:edit:salary-living'
        ? { sourceLocationId: 'salary', targetLocationId: 'living', allocationKind: 'fixed', monthlyAmountWon: 950_000, status: 'active' }
        : null,
      recordRecoveryDraft,
    } as unknown as AccountWorkspaceSession;
    render(<AccountDraftContext.Provider value={session}><AccountMapApp repositories={setup.repositories} /></AccountDraftContext.Provider>);
  ...
  it('clears a custom-purpose recovery when keeping the latest state after conflict', async () => {
    const setup = flowRepositories();
    const recordRecoveryDraft = vi.fn();
    const session = { readRecoveryDraft: () => null, recordRecoveryDraft } as unknown as AccountWorkspaceSession;
    render(<AccountDraftContext.Provider value={session}><AccountMapApp repositories={setup.repositories} /></AccountDraftContext.Provider>);
    fireEvent.click(screen.getByRole('button', { name: '세부 목적 추가' }));
    const dialog = screen.getByRole('dialog', { name: '세부 목적 추가' });
    fireEvent.change(within(dialog).getByRole('textbox', { name: '목적 이름' }), { target: { value: '교통비' } });
    fireEvent.change(within(dialog).getByRole('textbox', { name: '월 금액' }), { target: { value: '100000' } });
    setup.current().revision = 2;
    fireEvent.click(within(dialog).getByRole('button', { name: '추가' }));
    recordRecoveryDraft.mockClear();
  ...
  it('keeps the latest transfer after a conflict without replaying the local input', async () => {
    ...
    expect(recordRecoveryDraft).toHaveBeenCalledWith('account-map-transfer:map:edit:salary-living', null);
  });

  it('clears the scoped transfer recovery after a successful remove', async () => {
    const setup = flowRepositories();
    const recordRecoveryDraft = vi.fn();
    const session = {
      readRecoveryDraft: (key: string) => key === 'account-map-transfer:map:edit:salary-living'
    ...
    fireEvent.click(within(screen.getByRole('dialog', { name: '계좌 흐름 편집' })).getByRole('button', { name: '흐름 삭제' }));
