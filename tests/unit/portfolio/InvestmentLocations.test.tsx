...
function location(shortName: string = 'ISA'): FinancialLocation {
  return {
...
function createRepository(
  initial: FinancialLocation[] = [],
  { archiveReferenced = true }: { archiveReferenced?: boolean } = {},
) {
  let locations = [...initial];
  let listener: ((next: FinancialLocation[]) => void) | undefined;
  ...
  const repository: InvestmentLocationRepository & {
    ...
    create: vi.fn(async (input): Promise<LocationWriteResult> => {
      ...
      const created: FinancialLocation = {
