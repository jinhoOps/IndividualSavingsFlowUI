...
## Phase 7: 백테스트 시뮬레이터 대시보드 (Step 4 - Pending Polish)
...
**Current Status Note:**
- [v0.9.18] KRW 만원 단위 표기(`MoneyUtils.formatMan`) 적용 완료. 
- 대시보드 전반의 완성도 제고 및 사용자 경험 개선을 위한 추가 고도화 작업 진행 중.

**Plans:** 3 plans
- [x] 07-01-PLAN.md — 데이터 구조 확립 및 시뮬레이션 엔진 구현 (v0.9.6)
- [x] 07-02-PLAN.md — React 기반 대시보드 레이아웃 및 차트 컴포넌트 개발 (v0.9.6)
- [x] 07-03-PLAN.md — 사용성 개선 및 버그 수정 (v0.9.12)

---

## Phase 8: 지출 데이터 과거 비교 분석 (Issue #4 - In Progress)
...
3. **시각적 분석**: SVG 기반 Grouped Bar Chart를 통해 지출 변화를 직관적으로 비교. (진행 중)
4. **실시간 연동**: 현재 입력값 변경 시 비교 차트 및 요약 카드가 즉시 업데이트. (진행 중)

**Current Status Note:**
- [v0.9.18] 스냅샷 저장 및 관리 로직은 구현되었으나, 실제 UI(Grouped Bar Chart 등)는 아직 프로젝트에 반영되지 않음.
