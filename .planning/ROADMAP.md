...
## Phase 7: 백테스트 시뮬레이터 대시보드 (Step 4 - Issue #7, 개선 중)
...
3. **인터랙티브 대시보드**: React 기반의 반응형 UI 및 수익률 차트. (개선 필요)
4. **상대 비교 모드**: 특정 자산을 0%로 둔 상대 수익률 시각화 기능. (검증 필요)

**Plans:** 3 plans
- [x] 07-01-PLAN.md — 데이터 구조 확립 및 시뮬레이션 엔진 구현 (v0.9.6)
- [x] 07-02-PLAN.md — React 기반 대시보드 레이아웃 및 차트 컴포넌트 개발 (v0.9.6)
- [ ] 07-03-PLAN.md — 사용성 개선 및 버그 수정 (In-progress)

---

## Phase 8: 지출 데이터 과거 비교 분석 (Issue #4 - 미시작)

**Goal:** `DataHub`에 저장된 이전 스냅샷 데이터를 불러와 현재 지출과 카테고리별로 비교하는 시각화 도구를 제공한다.

**Plans:**
- [ ] 08-01-PLAN.md — 스냅샷 비교 엔진 및 막대 그래프 시각화 구현
