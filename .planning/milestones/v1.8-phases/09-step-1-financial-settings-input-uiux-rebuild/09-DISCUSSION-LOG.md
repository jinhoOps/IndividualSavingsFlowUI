...
| Question | Options Considered | User's choice |
|----------|--------------------|---------------|
| 큰 금액을 한국식 단위로 전환할 때 어느 정도까지 표시할까요? | 기존 전체 단위 표시; 가장 큰 단위 + 한 단계 아래 단위; 항상 원 단위 포함 | 가장 큰 단위와 한 단계 아래 단위까지만 표시 |

**Notes:** 억 단위로 전환되면 만원 단위까지만, 조 단위로 전환되면 억 단위까지만 보여준다. 큰 단위 표시에서 천원/원처럼 더 작은 단위까지 길게 붙이지 않는다.
