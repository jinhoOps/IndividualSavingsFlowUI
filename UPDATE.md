...
- 성실 총자산을 저축/투자 성장률 0% + 보유자산 성장률=인플레이션-0.5%(실질 기준) 시나리오로 정리.
...
- 노드/링크 편집 UI를 우측 드로어로 분리하고 목록 패널은 리스트 중심으로 단순화.
- 드로어 오픈/닫기(ESC/배경 클릭 포함)와 바디 스크롤 잠금을 추가.
- 도넛 차트에 잔여금/적자 외곽 링과 범례 항목을 추가.
- 성실 총자산은 전체 자산에 인플레이션-0.5% 성장률을 적용하도록 분리해 실질 총자산과 겹침을 완화.
- 보유자산 성장률 입력을 제거하고 인플레이션 + 보유자산 투자수익률 자동 계산으로 전환.
