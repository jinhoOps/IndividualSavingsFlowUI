...
### 2026-04-11 (v0.2.3 - Antigravity 에이전트 연동 초기 설정)
- `chore`: Antigravity 에이전트 전용 설정 폴더(`.gemini/`) 구축.
- `docs`: `.gemini/policy.json` 및 `.gemini/SKILL.md` 생성을 통해 에이전트 작업 원칙 및 프로젝트 맥락 정의.
- `chore`: `.gemini/` 폴더를 `.gitignore`에 추가하여 환경별 로컬 설정 유지.
- `docs`: `UPDATE.md`의 운영 원칙을 `SKILL.md`로 통합하여 에이전트 가독성 향상.

### 2026-04-11 (v0.2.2 - Step2 UX 고도화: 드래그 앤 드롭 및 차트 가시성 개선)
- `ux`: 종목(자산군) 순서 변경 방식을 ↑/↓ 버튼에서 **전체 행 드래그 앤 드롭**으로 전환.
  - 데스크톱: 인풋 제외 영역 드래그로 즉시 이동.
  - 모바일: **롱 프레스(0.5초)** 감지 시 드래그 모드 활성화로 스크롤 간섭 방지.
- `ux`: 종합 도넛 차트 가시성 개선.
  - **소액 종목 그룹화**: 전체 비중 1% 이하 종목들을 `기타 n종`으로 자동 통합.
  - **데이터 라벨링**: 차트 슬라이드 위에 종목명과 비중(%) 라벨을 직접 표시해 가독성 확보.
- `chore`: TODO.md 상태 업데이트 및 코드 리팩토링.
