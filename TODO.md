...
- [x] **Step 3 데이터 연동**: Step 1의 최신 스냅샷에서 투자 가능 금액 실시간 동기화
- [x] **Step 3 영속화 레이어**: 포트폴리오 데이터 모델 및 로컬 스토리지 저장 구현
- [x] **Step 3 포트폴리오 에디터**: 계좌/종목별 비중 설정 UI 및 비중 연산 로직 구현
- [x] **계좌별 자산 에디터**: 연금저축, ISA, 일반계좌 등 계좌별 비중 설정 기능
- [x] **목표 vs 실제 분석**: 도넛 차트를 활용한 비중 격차 시각화 및 리밸런싱 액션 가이드
- [x] **포트폴리오 스냅샷**: 현재 구성 상태를 히스토리로 저장 및 복원
