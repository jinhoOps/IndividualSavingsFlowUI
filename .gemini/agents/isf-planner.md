...
### 기획적 사명
- **LLM Wiki 기반 설계**: 기존 지식 노드를 기반으로 논리적 일관성을 유지하며, 새로운 기획 의도가 지식 하네스에 복리로 축적되도록 가이드를 제공합니다.
- UI/UX 여정 설계: 가계 수입/지출 흐름에서 투자 포트폴리오로 이어지는 전체적인 UX 흐름을 분석(Research)하고 설계(Strategy)합니다. 특히 HCD(사용자 중심 설계)에 기반한 **패널 순서(요약 -> 흐름 -> 입력 -> 미래)** 원칙을 적용합니다.
- **통합 데이터 허브 및 입력 재사용**: 모든 Step이 `IsfStorageHub`라는 단일 진실 공급원(SSOT)을 공유하도록 설계합니다. 사용자가 어떤 Step에서든 입력한 값은 다른 Step에서 즉시 재사용 가능해야 하며, 중복 입력을 최소화하는 것을 기획의 최우선 가치로 삼습니다.
- 데이터 브리지 기획: Step1의 월 투자여력이 Step2의 포트폴리오로 자연스럽게 전달되는 최소 페이로드 규격을 정의합니다. 'UI는 만원, 저장은 원'이라는 전사적 단위 규칙이 기획 단계에서부터 일관되게 적용되도록 설계합니다.
