---
type: node
title: "UI Standards Reference"
description: "UI Standards Reference reference documentation."
...
4. **FeedbackManager (`shared/components/feedback-manager.js`)**
   ...
   - **3종 랜덤 애니메이션 플로팅 펜딩 바 (Pending Bar)**: Step 3 포트폴리오 모달 변경 감지 시 화면 하단에 고정 노출되는 미니멀리즘 펜딩 바. 최초 노출 시 3가지 애니메이션(`anim-slide-up`, `anim-fade-scale`, `anim-bounce-in`) 중 하나가 무작위 적용되며, '취소' 클릭 시 모달이 닫히지 않고 계속 편집 가능한 상태를 유지합니다. 메시지가 생략된 버튼 위주의 미니멀 UI입니다.
   - `notifyAutoSave`: 자동 저장 상태 알림.
5. **Taxation Warnings (정책 기반 경고)**
   - [Financial_Taxation_Reference](./financial_taxation_reference.md)에 정의된 안전 마진(1,900만/3,400만) 기준에 따라 `warn`/`crit` 레벨의 피드백을 제공합니다.
   - **글로벌 과세경고 LED**: 복잡한 뱃지 대신 화면 상단 헤더 로고 그룹 내에 단일 글로벌 LED 인디케이터(`globalFinancialIncomeIndicator`)를 배치하여 종합과세 대상 상태를 일목요연하고 깔끔하게 표기합니다.
...
### 적립식 포트폴리오 관리 (Step 3 고도화 - v1.8)
- **포트폴리오 에디터 폼**: 만들기 화면(`portfolioCreator`)은 기본적으로 숨겨져 있으며, `+ 포트폴리오 추가` 및 `취소` 버튼으로 부드럽게 토글됩니다.
- **주기 선택기**: 가로 세그먼트 형태의 주기 선택기(매일, 매주, 매월)를 도입해 세련된 UX를 구성합니다.
- **1,000원 단위 +/- 올림/내림 가감 보정**: 금액 인풋 좌우 가감 버튼 클릭 시 1,000원 단위가 아닐 경우 올림/내림 보정(plus 시 `Math.ceil`, minus 시 `Math.floor`) 공식을 적용하여 정합성을 보정합니다.
- **상시 활성화 생성 버튼 & 디테일 에러 피드백**: 생성 버튼은 항상 클릭할 수 있으며, 클릭 시 미입력/오입력 원인(종목 개수 부족, 최소 금액 미준수 등)을 정확히 콕 집어 디테일 경고창으로 제공하여 인지 부하를 줄입니다.
- **금융 현실성 반영 추이 시뮬레이션**: 매일 적립 시 365일 배수 대신 금융 영업일 기준(연 240일)으로 환산하여 현실적인 1년 누적 금액 추이를 막대 그래프로 시뮬레이션하고 힌트 문구를 표출합니다.
- **디자인 테마 피드백**: 포트폴리오 카드 호버 시 **Sunset Orange** 톤의 테두리 광채 효과(`box-shadow`)와 Y축 -2px 부유 애니메이션을 적용해 프리미엄 디자인 감성을 통일합니다.

## 입력 폼 및 실시간 한글 금액 힌트 (v1.7)
- **1원 단위 직접 인풋**: 모든 자산 및 금액 입력 필드는 '원(KRW)' 단위를 기준으로 직접 입력받아 계산 오류를 방지합니다.
- **실시간 한글 금액 힌트 (.realtime-won-hint)**: 입력 중인 인풋 필드 바로 하단에 실시간으로 `convertToKoreanWon` 헬퍼가 적용된 한글 금액 힌트(예: `500만 원`, `1억 2,000만 원`)를 동적으로 노출하여, 1원 단위 입력으로 인한 자릿수 인식 부하를 해결합니다.
- **차단 필터**: 금리, 수익률, 성장률 등 비율(%)을 다루는 필드(`isRateField`)에는 힌트가 중복 노출되지 않도록 차단 필터를 장착합니다.
...
---
*연결 노드:* [Architecture_Reference](./architecture_reference.md), [Data_Model_Reference](./data_model_reference.md), [Operating_Principles](./operating_principles.md)
