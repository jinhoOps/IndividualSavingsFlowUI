...
- `shared/`: 여러 앱에서 공유하는 핵심 모듈.
  ...
  ### 중앙 데이터 허브 철학
  - **Single Hub, Multi-Step**: `IsfStorageHub`는 단순한 저장소가 아니라, 프로젝트 내 모든 Step이 공유하는 **중앙 컨텍스트 허브**입니다. 
  - **입력 데이터 재사용**: 사용자가 한 Step에서 입력한 핵심 데이터(예: 월 투자 여력)는 허브를 통해 모든 Step에서 즉시 참조 및 재사용됩니다. 새로운 Step을 추가할 때 기존 허브에 있는 데이터를 우선 활용하여 사용자 입력 피로를 낮추는 것이 아키텍처의 핵심 목표입니다.
  ...
  - **지능형 자동 연동 (v0.7.3)**: Step 2 진입 시 Step 1의 '월 투자 여력' 데이터를 별도의 확인 절차 없이 **자동으로 불러와 즉시 시뮬레이션에 반영**합니다. 사용자는 이미 최신 데이터가 적용된 그래프를 바로 보게 됩니다. 만약 사용자가 이 자동으로 연동된 금액을 수동으로 변경하려고 시도할 때만 "연동된 값을 수정하시겠습니까?"라는 확인 팝업/배너를 통해 의사를 묻습니다. (참고: [[Data_Bridge_Import_Pattern]])
