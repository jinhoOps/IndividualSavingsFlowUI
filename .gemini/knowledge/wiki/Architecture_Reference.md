...
- `shared/`: 여러 앱에서 공유하는 핵심 모듈.
  ...
  - **지능형 자동 연동 (v0.7.4)**: Step 2 진입 시 Step 1의 '월 투자 여력' 데이터를 별도의 확인 절차 없이 **자동으로 불러와 즉시 시뮬레이션에 반영**합니다. 사용자는 이미 최신 데이터가 적용된 그래프를 바로 보게 됩니다. 만약 사용자가 이 자동으로 연동된 금액을 수동으로 변경하려고 시도할 때만 "연동된 값을 수정하시겠습니까?"라는 확인 팝업/배너를 통해 의사를 묻습니다. (참고: [[Data_Bridge_Import_Pattern]])
