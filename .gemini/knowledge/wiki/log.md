...
## [2026-06-12] ingest | Phase 4: 계좌 간 이체 시각화 및 UX 개선 기획 및 설계 수립 (v0.11.26)
- **목적**: 계좌 간 N:N 이체 흐름을 명확히 추적하기 위한 SVG 기반 자금 흐름 네트워크 차트(Network Map) 도입, 수동 이체 설정 및 편집 UI 설계, 단계 전환 슬라이드 애니메이션 및 전역 마이크로 호버 모션 수립.
- **주요 합의사항**:
  - **Account Flow Network Map (D-01)**: 생키 하단에 독립된 SVG 흐름 지도를 신설하여 계좌 노드들의 원형/격자 배치, 방향성 화살표, 펄스 애니메이션(흐름선 파티클) 및 노드 호버 시 관련 경로만 하이라이트하는 포커스 필터링 구현.
  - **Manual Transfer Rules & UI (D-02)**: `Step1State` 모델에 `transfers` 배열을 영속화하여 사용자가 "출발 계좌 ➔ 도착 계좌, 금액" 수동 이체 룰을 직접 제어할 수 있는 입력 폼 및 목록 UI 제공.
  - **Step Transitions & Global Motions (D-03)**: Step 뷰포트 전환 시 좌우로 부드럽게 흘러가는 CSS 페이드 & 슬라이드 애니메이션 적용 및 Step 1 전체 컴포넌트에 호버 부유 모션 최적화.
  - **Real-time Safety Margin Reactivity (D-04)**: 저축/투자 변경에 따른 종합과세 한도 초과 경고 인디케이터(링/뱃지)의 즉각적인 실시간 렌더링 라이프사이클 공고화.
- **결과**: `.planning/phases/04-account-transfer-ui-ux-polish/` 아래 `04-CONTEXT.md` 및 `04-01-PLAN.md` 기획 산출물 확보 및 v0.11.26 버전 빌드 확인 완료.
