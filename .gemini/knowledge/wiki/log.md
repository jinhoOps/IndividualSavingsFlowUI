...
## [2026-05-06] bump | Step 2 프리셋 명칭 표시 UI 개선 및 패치 (v0.9.1)
...
- **변경 사항**:
    ...
    - **버전 패치**: 전역 버전을 v0.9.0에서 v0.9.1로 업데이트 (package.json, sw.js, utils.js 등 6종).
- **영향**: 사용자 경험(UX) 개선 및 전략 선택 상태의 가독성 향상.

## [2026-05-06] ingest | Modern Hybrid 아키텍처 전환 및 금융 시뮬레이션 고도화 완결 (v0.9.0)
- **목적**: `feat/modern-poc` 브랜치의 모든 성과를 `main`으로 머지하고, 프로젝트를 현대적 인프라와 정밀한 금융 로직이 결합된 v0.9.0 체제로 전환함.
- **주요 변경사항**:
    - **통합 머지**: Vite, TS, Tailwind v4 기반의 현대화된 인프라를 메인 라인에 통합.
    - **금융 정책 확립**: 4% 안전 마진 정책을 수립하고 `TAX_CONFIG`를 통해 소득세 계산 로직을 투명화함.
    - **버전 관리**: 프로젝트 전반의 버전을 v0.9.0으로 일원화하고 피처 브랜치 정리 수행.
