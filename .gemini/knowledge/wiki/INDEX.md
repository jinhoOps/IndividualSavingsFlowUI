...
이 파일은 `wiki-librarian` 에펠(스킬)을 통해 관리되는 프로젝트 위키의 전체 대동여지도(최상위 목차 파일)입니다. 에이전트는 작업을 시작할 때 이 파일을 통해 전체 지식의 위상(Topology)을 파악하고 필요한 노드로 점프(Context Loading)해야 합니다.
