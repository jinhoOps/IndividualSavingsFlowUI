...
| isf-strategist | 금융 로직/데이터 전략 및 검증 | Gemini 3 | 0.2 |
...
[1. Planner] -- 기획 산출물 생성
      |
      v
[2. Strategist] -- 금융 타당성 및 데이터 현실성 검증
      |
      v
[3. Developer] -- 구현 및 스프린트 계약 체결
      |
      v
[4. Evaluator] -- 품질 검증 및 승인/반려
      |
     / \
    v   v
 [승인]  [반려] --> Planner로 루프백 (결함 환류)
```

### 1단계: 기획 (Planner)
... (기존 내용 유지)

### 2단계: 전략 및 검증 (Strategist)

- 입력: Planner의 기획 산출물 및 사용자 요구사항
- 산출물:
  - 금융 로직 타당성 검토서 (CAGR/IRR 수식 등)
  - 데이터 현실성 체크리스트 (Mock 데이터 의존도 및 실제 데이터 전환 필요성)
  - 거시경제 시나리오 반영 가이드
- 검증 기준: "이 기획이 실제 금융 환경에서 오해 없이 작동하는가?"
...
- 검증 항목:
  ...
  - **버전 정합성 (Triple Sync Check)**:
    - `sw.js`, `manifest.webmanifest`, `app.js`의 버전 번호가 일치하는가?
  - **시스템 무결성 및 단위 일관성**:
...
- Evaluator가 불합격 판정을 내리면, 결함 목록을 Planner에게 환류(Feedback)하여 기획 단계부터 재검토합니다.
- Planner는 기획을 보완/수정하고, 이후 Strategist -> Developer -> Evaluator 순으로 워크플로우를 재실행합니다.
- 이는 기능 구현의 기술적 결함뿐만 아니라 기획의 타당성까지 전 단계에서 재검증하기 위함입니다.
- 루프를 3회 수행하는경우 부족한 부분을 정리해서 유저에게 리포팅하고 작업을 중단합니다.
...
- 새로운 라이브러리 추가 시 'Modern Hybrid (No-build Oriented)' 정책과의 호환성을 먼저 검증하십시오.
